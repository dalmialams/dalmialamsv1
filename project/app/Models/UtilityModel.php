    ...
    public function makeMenu($current_user)
    {
        ...
        Menu::make('MyNavBar', function ($menu) {
            ...
            $menu->group(array(), function ($entry_menu) {
                ...
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("registration_access", $this->current_user_id)) {
                    $entry_menu->group(array('prefix' => 'land-details-entry'), function ($master) {
                        $master->entry->add('Land Sync', array('url' => 'registration/sync', 'secure' => false))->data('permission', array('registration_sync'));
                    });
                }

                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_document_access", $this->current_user_id)) {
                    $entry_menu->group(array('prefix' => 'land-details-entry'), function ($master) {
                        $master->entry->add('Land Document', array('url' => 'land-document/list', 'secure' => false));
                        //->data('permission', array('land_document_add'));
                    });
                }
            ...
            $menu->group(array(), function ($trnxn_menu) {
                ...
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("hypothecation_access", $this->current_user_id)) {
                    $trnxn_menu->group(array('url' => '#', 'secure' => false), function ($master) {
                        $master->trnxn->add('Mortgage Land', array('url' => 'transaction/hypothecation/add', 'secure' => false))->data('permission', array('hypothecation_add'));
                    });
                }
            ...
            $menu->group(array(), function ($mis_menu) {
                ...
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_inspection_mis", $this->current_user_id)) {
                    $mis_menu->group(array('prefix' => 'mis'), function ($master) {
                        // $master->mis->add('Transaction Details', array('url' => '', 'secure' => true))->data('permission', array(''));
                        $master->mis->add('Land Inspection Details', array('url' => 'land_inspection', 'secure' => false));
                    });
                }

                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("mortgage_land_mis", $this->current_user_id)) {
                    $mis_menu->group(array('prefix' => 'mis'), function ($master) {
                        // $master->mis->add('Transaction Details', array('url' => '', 'secure' => true))->data('permission', array(''));
                        $master->mis->add('Mortgage Land Details', array('url' => 'mortgage_land', 'secure' => false));
                    });
                }
            ...
            $menu->group(array(), function ($admin_menu) {
                ...
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("search_log_access", $this->current_user_id)) {
                    $admin_menu->group(array(''), function ($master) {
                        $master->admin->add('Search Log', array('url' => 'master/search/search-log', 'secure' => false))->data('permission', array());
                    });
                }
