...
class RegistrationController extends Controller
{
    ...
    protected $validationRules = [
        ...
        'registration.sap_asset_code' => 'required',
    ...
    protected $field_names = [
        ...
        'registration.sap_asset_code' => 'SAP Asset Code',
    ...
    protected $messages = [
        ...
        'registration.sap_asset_code.alpha_num' => 'SAP Asset Code must be alphanumeric',
        'registration.sap_asset_code.size' => 'SAP Asset Code must be 12 digit',
    ];
     ...
     public function sync(Request $request)
    {
        ...
        //  $this->data['title'] = 'Dalmia-lams::Registration/Add';
        $this->data['pageHeading'] = 'Land Record <span class="text-danger" >Sync</span>';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/select2/select2.js';
        $this->data['data']['jsArr'][] = 'assets/js/formAjaxPlugin.js';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/validation/jquery.validate.js';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/bootstrap-datepicker/bootstrap-datepicker.js';
        ...
        $this->data['data']['jsArr'][] = 'assets/js/autoNumeric-min.js';
        //t($this->data);exit;
        return view('admin.LandEntryManagement.Registration.sync', $this->data);
    }
    ...
    public function regView()
    {
        ...
        if ($this->data['reg_uniq_no'] && $this->data['viewMode'] == 'true') {
            ...
            $reg_uniq_no = $this->data['reg_uniq_no'];
            $reg_info = RegistrationModel::find($reg_uniq_no)->toArray();
            $totalcost=  RegistrationModel::where('sap_asset_code',$reg_info['sap_asset_code'])->sum('tot_cost');
            $this->data['reg_data'] = $reg_info;
    ...
    public function regList(Request $request)
    {
        ...
        if ($posted_data) {
            ...
            if (!empty($allSearchData['state_id'])) {
                ...
                $stId = $allSearchData['state_id'];
                $assigned_districts = "";
                if ($this->user_type !== 'admin') {
                    $states = StateModel::whereRaw("id in ('$stId')")->where(['fl_archive' => "N"])
                    ...
                    $assigned_districts = isset($states[0]['assigned_districts'][0]['dist_id_str']) ? $states[0]['assigned_districts'][0]['dist_id_str'] : "";
                }
                ...
                if ($this->user_type !== 'admin') {
                    ...
                    $cond .= " AND district_id in ($assigned_districts) ";
                } else {
            ...
            // $sql = "SELECT * FROM public.reg_cons_view WHERE {$cond}";
            // $sumSql = "SELECT SUM(purchased_area_in_acre) as purchased_area_in_acre , SUM(payment_sum) as total_cost FROM public.reg_cons_view WHERE {$cond}";
            ...
            $grand_total_tot_cost = RegistrationModel::join('T_PAYMENT', 'T_PAYMENT.registration_id', '=', "T_REGISTRATION.id", 'left')->selectRaw('sum(amount) as grand_total_tot_cost')->whereRaw($cond)->get()->toArray();

            $this->data['registration'] = $res;
            $this->data['sumResult'] = !empty($grand_total_tot_cost[0]) ? $grand_total_tot_cost[0]['grand_total_tot_cost'] : 0;
    ...
    public function processData(Request $request)
    {
        ...
        if ($id) {
            ...
            $existsData = RegistrationModel::where(['state_id' => $data_to_insert['state_id']])->orderBy('id', 'desc')->take(1)->pluck('reg_id')->toArray();
            $sate_prefix = StateModel::where(['id' => "{$data_to_insert['state_id']}"])->pluck('state_prefix')->toArray();
    ...
    public function transactionDetails()
    {
        ...
        $data['pageHeading'] = 'Land Record <span class="text-danger" >Transaction Details</span>';
        $data['title'] = 'Dalmia-lams::Transaction';
        $data['section'] = 'transaction';
        $data['data']['jsArr'][] = 'assets/plugins/forms/select2/select2.js';
        $data['data']['jsArr'][] = 'assets/js/formAjaxPlugin.js';

        $data['data']['jsArr'][] = 'assets/plugins/forms/bootstrap-datepicker/bootstrap-datepicker.js';
        $data['data']['jsArr'][] = 'assets/js/autoNumeric-min.js';
        $data['data']['jsArr'][] = 'assets/plugins/forms/bootstrap-filestyle/bootstrap-filestyle.js';

        $data['data']['jsArr'][] = 'assets/plugins/tables/datatables/jquery.dataTables.js';
        $data['data']['jsArr'][] = 'assets/plugins/tables/datatables/dataTables.tableTools.js';
        $data['data']['jsArr'][] = 'assets/plugins/tables/datatables/dataTables.bootstrap.js';
        $data['data']['jsArr'][] = 'assets/plugins/tables/datatables/dataTables.responsive.js';
        $data['data']['jsArr'][] = 'assets/js/pages/tables-data.js';
        $data['include_script_view'] = 'admin.LandEntryManagement.Registration.script';
        ...
        $data['dataPresent'] = '';
        $data['reg_info'] = ($reg_info) ? $reg_info : '';
        ...
        $data['conversion_to_parent_company_details'] = $conversion_to_parent_company_details;
        /// land ceilling //////
        ...
        $data['ceilings'] = $ceilings = ($ceiling_id_str) ? \App\Models\Transaction\LandCeilingModel::whereRaw("id in ($ceiling_id_str)")->where(['fl_archive' => 'N'])->with('getCeilingSurveys')->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['exchanges'] = $exchanges = ($exchange_id_str) ? \App\Models\Transaction\LandExchangeModel::whereRaw("id in ($exchange_id_str)")->where(['fl_archive' => 'N'])->with('getExchangedSurveys')->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['conversions'] = $conversions = ($conversion_id_str) ? \App\Models\Transaction\LandConversionModel::whereRaw("id in ($conversion_id_str)")->where(['fl_archive' => 'N'])->with('getConversionSurveys')->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['reservations'] = $reservations = ($reservation_id_str) ? \App\Models\Transaction\LandReservationModel::whereRaw("id in ($reservation_id_str)")->where(['fl_archive' => 'N'])->with('getReservedSurveys')->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['minings'] = $minings = ($mining_id_str) ? \App\Models\Transaction\MiningLeaseModel::whereRaw("id in ($mining_id_str)")->where(['fl_archive' => 'N'])->with('getMiningSurveys')->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['mutations'] = $mutations = ($mutation_id_str) ? \App\Models\Transaction\MutationModel::whereRaw("id in ($mutation_id_str)")->where(['fl_archive' => 'N'])->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['disputes'] = $disputes = ($dispute_id_str) ? \App\Models\Transaction\DisputesModel::whereRaw("id in ($dispute_id_str)")->where(['fl_archive' => 'N'])->orderBy('id', 'DESC')->get()->toArray() : '';
        ...
        $data['registration_converted_flag'] = $this->data['registration_converted_flag'];
        $data['validator'] = $this->data['validator'];
        return view('admin.LandEntryManagement.Registration.transaction-details', $data);
    }
    ...
    public function regauditView(Request $request)
    {
        ...
        $results = DB::select("SELECT * FROM audit.logged_actions WHERE row_data->'id' = '$reg_id' AND table_name='$table'");
        ...
        $state = DB::table('T_STATE')->get();
        $BLOCK = DB::table('T_BLOCK')->get();
        $DISTRICT = DB::table('T_DISTRICT')->get();
        $VILLAGE = DB::table('T_VILLAGE')->get();
        $USER = \App\Models\User\UserModel::get();
        $ROLES = DB::table('T_ROLES')->get();
        $FAMILY = DB::table('T_FAMILY')->get();
        ...
        foreach ($results as $k => $infos) {
            ...
            } else if ($infos->session_user_name != '') {
                $html .= $infos->session_user_name;
            } else {
                $html .= 'System';
            }
            $html .= '</td>
    ...
    public function post_data_to_sap(Request $request ) {
        $posted_data = $request->all();
      
        $csrf_response = $this->get_sap_csrf_token();
        if (!$csrf_response) {
        ...
        //$sap_url = 'https://gatewayqa.dalmiabharat.com/sap/bc/zdms/cvondms?sap-client=900';
        $sap_url = 'https://gatewayqa.dalmiabharat.com/zapi/zfi/lms?sap-client=900';

        $data = [
            "CompanyCode" => $posted_data['company_code'],
            "Asset" =>$posted_data['sap_code'],
            "Year" => date("Y")        
        ];
        ...
        curl_setopt($ch, CURLOPT_USERPWD, "lms:Worldofuser*54321");
        ...
        $response_body = substr($response, strpos($response, "\r\n\r\n") + 4); // strip headers
         $data_array = json_decode($response_body, true);
        //  print_r($data_array);die;
        $data_to_insert = [];
        foreach($data_array as $res){
            $data_to_insert['created_at'] = Carbon::now();
            $data_to_insert['sap_asset_code'] = $res['Asset'];
            $data_to_insert['sap_company_code'] = $posted_data['company_code'];
            $data_to_insert['regn_no'] = '';
            $data_to_insert['tot_cost'] = $res['GrossBookValue'];
            $data_to_insert['tot_area'] = $res['Quantity'];
            $data_to_insert['tot_area_unit'] = $res['UOM'];
        }
        ...
        $this->data['data']['jsArr'][] = 'assets/plugins/tables/datatables/dataTables.responsive.js';
        $this->data['data']['jsArr'][] = 'assets/js/pages/tables-data.js';
        $this->data['reg_uniq_no']= $insert_id;
        $this->data['viewMode'] == true;
        return redirect('land-details-entry/registration/view?reg_uniq_no=' . $insert_id.'&view=true')->with('message', $msg);

    }


    public function get_sap_csrf_token(){
        $sap_url = 'https://gatewayqa.dalmiabharat.com/zapi/zfi/lms?sap-client=900';

        $headers = [
            "x-csrf-token: Fetch",
            "Authorization: Basic " . base64_encode("lms:Worldofuser*54321"),
            "Content-Type: application/json"
        ];
        ...
        curl_close($ch);
        // 🔍 Extract CSRF token
        preg_match("/x-csrf-token:\s*(\S+)/i", $header, $matches);
        $csrf_token = isset($matches[1]) ? trim($matches[1]) : null;
       
        preg_match_all('/Set-Cookie:\s*([^;]+);/i', $header, $cookie_matches);
        $cookies = implode("; ", $cookie_matches[1]); // e.g., sap-usercontext=something; MYSAPSSO2=xyz

       
        if (!$csrf_token || $csrf_token == 'Required') {
            echo "❌ CSRF Token fetch failed or token not provided.";
            return null;
        }

       // echo "✅ CSRF Token: $csrf_token"; // Or return it
        return [
            'token' => $csrf_token,
            'cookies' => $cookies
        ];
