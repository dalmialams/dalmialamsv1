...
use App\Models\Transaction\LandConversionModel;
use App\Models\Transaction\LandConversionSurveyModel;
 ...
 * @author user-98-pc
 */
class LandConversionController extends Controller {

    //put your code here

    protected $validationRules = [
        'land_conversion.trxn_type' => 'required',
        'land_conversion.state_id' => 'required',
        'land_conversion.district_id' => 'required',
        'land_conversion.block_id' => 'required',
        'land_conversion.village_id' => 'required',
    ];
    protected $field_names = [
        'land_conversion.trxn_type' => 'Transaction Type',
        'land_conversion.state_id' => 'State',
        'land_conversion.district_id' => 'District',
        'land_conversion.block_id' => 'Block/Taluk',
        'land_conversion.village_id' => 'Village',
    ];
    ...
    public function __construct() {
        ...
        $this->upload_dir = 'assets/uploads/ConversionSurveyDocs';
        $this->data['data']['cssArr'] = [];
        $this->data['data']['jsArr'] = [];
        $this->data['title'] = 'Dalmia-lams::Transaction-Land Conversion';
        ...
        $this->data['include_script_view'] = 'admin.Transaction.LandConversion.script';
    ...
    public function add(Request $request) {
        ...
        $this->data['pageHeading'] = 'Land Conversion <span class="text-danger" >Add</span>';
        $this->data['title'] = 'Dalmia-lams::Transaction-land-conversion-Add';
        ...
        //Land Conversion Lists

        if (isset($this->assgined_districts_str) && !empty($this->assgined_districts_str)) {
            $res = LandConversionModel::whereRaw(" district_id in ($this->assgined_districts_str)")->where(['fl_archive' => 'N'])->get()->toArray();
        } else {
            $res = LandConversionModel::where(['fl_archive' => 'N'])->get()->toArray();
        }

        $this->data['land_conversions'] = $res;
        $this->data['id'] = '';
        $this->data['type'] = [
            '' => 'Select',
            'Y' => 'Applied',
        ...
        return view('admin.Transaction.LandConversion.add', $this->data);
    }

    public function edit(Request $request, $id = null) {

        if (!$id) {
            return redirect('transaction/land-conversion/add');
        } else {
            $exists = LandConversionModel::where(['id' => $id])->get()->toArray();
            $dist_id = LandConversionModel::where(['id' => $id])->value('district_id');
            if ($this->user_type == 'admin') {
                if (empty($exists)) {
                    return redirect('transaction/land-conversion/add');
                }
        ...
        $this->data['pageHeading'] = 'Land Conversion (' . $id . ') <span class="text-danger" >Edit</span>';
        $this->data['title'] = 'Dalmia-lams::Transaction-land-conversion-Add';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/select2/select2.js';
        $this->data['data']['jsArr'][] = 'assets/js/formAjaxPlugin.js';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/validation/jquery.validate.js';
        ...
        if ($id) {
            ...
            $land_conversion_data = LandConversionModel::where(['id' => $id])->get()->toArray();
            $trxn_type = $land_conversion_data[0]['trxn_type'];
            $trxn_type = ($trxn_type == 'N') ? 'Y' : 'N';
            $this->data['land_conversion_data'] = $land_conversion_data[0];

            $conversion_surveys = LandConversionModel::find($id)->getConversionSurveys->toArray();
            $this->data['conversion_surveys'] = $conversion_surveys;
            if (empty($conversion_surveys)) {
                return redirect('transaction/land-conversion/add');
            }

            if ($land_conversion_data[0]['state_id']) {
                $this->data['district_info'] = $this->getAllDistrict($land_conversion_data[0]['state_id']);
            }
            if ($land_conversion_data[0]['district_id']) {
                $this->data['block_info'] = $this->getAllBlock($land_conversion_data[0]['district_id']);
            }
            if ($land_conversion_data[0]['block_id']) {
                $this->data['village_info'] = $this->getAllVillage($land_conversion_data[0]['block_id']);
            }
            if ($land_conversion_data[0]['village_id']) {
                $existing_surveys = LandConversionSurveyModel::where(['conversion_id' => $id])->select('survey_id')->get()->toArray();
                $this->data['survey_info'] = $survey_info = $this->conversionSurvey($land_conversion_data[0]['village_id'], null, $trxn_type);
            }
        }
    ...
    public function processData(Request $request) {
        ...
        $posted_land_conversion_data = isset($posted_data['land_conversion']) ? $posted_data['land_conversion'] : '';
        $transaction_type = isset($posted_land_conversion_data['trxn_type']) ? $posted_land_conversion_data['trxn_type'] : '';
        unset($posted_land_conversion_data['survey_id']);
        ...
        if ($id) {
            ...
            if ($this->user_type !== 'admin') {
                if (!$this->hasPermission('land_conversion_edit')) {
                    return redirect('unauthorized-access');
                }
            }

            if ($validator->fails()) {
                return redirect('transaction/land-conversion/edit/' . $id)
                                ->withErrors($validator, 'land_conversion')
                                ->withInput();
            }

            if ($file_error > 0) {
                $msg = UtilityModel::getMessage('Some files are not valid!', 'error');
                return redirect('transaction/land-conversion/edit/' . $id)->with('message', $msg);
            }
            ...
            $data_to_update = $posted_land_conversion_data;
            ...
            $update = LandConversionModel::where(['id' => $id])->update($data_to_update);
            $this->addOrEditConversionSurveys($posted_data, $selected_surveys, $id, $path_arr, $ext_arr, $transaction_type, 'edit');
            $msg = UtilityModel::getMessage('Land Conversion data updated successfully!');

            return redirect('transaction/land-conversion/edit/' . $id)->with('message', $msg);
        } else {

            if ($this->user_type !== 'admin') {
                if (!$this->hasPermission('land_conversion_add')) {
                    return redirect('unauthorized-access');
                }
            }
            if ($validator->fails()) {
            ...
            if ($file_error > 0) {
                $msg = UtilityModel::getMessage('Some files are not valid!', 'error');
                return redirect('transaction/land-conversion/add')->with('message', $msg);
            }
            ...
            $data_to_insert = $posted_land_conversion_data;
            ...
            $insert_id = LandConversionModel::insertGetId($data_to_insert);
            $msg = UtilityModel::getMessage('Land Conversion data saved successfully!');
            if ($insert_id) {
                $this->addOrEditConversionSurveys($posted_data, $selected_surveys, $insert_id, $path_arr, $ext_arr, $transaction_type, 'add');
                return redirect('transaction/land-conversion/add')->with('message', $msg);
            }
        }
    }

    public function addOrEditConversionSurveys($posted_data, $selected_surveys, $conversion_id, $path_arr, $ext_arr, $transaction_type, $mode) {
        if ($posted_data && $selected_surveys && $conversion_id) {
            $survey_data['conversion_id'] = $conversion_id;
            if ($selected_surveys) {
                foreach ($selected_surveys as $key => $value) {
                    if ($mode == 'add' && ($transaction_type == 'N' || $transaction_type == 'Y')) {
                        $existing_survey = LandConversionSurveyModel::where(['survey_id' => $value])->get()->toArray();
                        if (!empty($existing_survey)) {
                            $delete_survey = LandConversionSurveyModel::where(['survey_id' => $value])->delete();
                        }
                    }
                    ...
                    if (empty($existing_survey)) {
                        ...
                        $insert_survey_id = LandConversionSurveyModel::insertGetId($survey_data);
                    } else {
                        ...
                        $update = LandConversionSurveyModel::where(['survey_id' => $value])->update($survey_data);
                    }
                    $transaction_type = ($transaction_type == 'Y') ? 'Y' : 'O'; //Default N, If Applied then Y, If Obtained then O
                    $update_land_conversion_status_in_survey_table = SurveyModel::where(['id' => $value])->update(['land_conversion_status' => $transaction_type]);
                }
            }
        }
    }
    ...
    public function dropDown(Request $request, $option = true, $json = true) {
        ...
        if ($typeList == 'survey_id') {
            ...
            $surveys = $this->conversionSurvey($village_id, 'ajax', $trxn_type, $from_mis);
    ...
    public function conversionSurvey($village_id = null, $type = null, $trxn_type = 'N', $from_mis = null) {
        ...
        if (!$from_mis) {
            if ($village_id && $type == 'ajax') {
                ...
                if ($registration_ids) {
                    $survey_list = SurveyModel::where(['land_conversion_status' => $trxn_type])->whereRaw("registration_id in ($registration_ids)")->orderBy('id')->select('id as option_code', 'survey_no as option_desc')->get()->toArray();
                } else {
                    $survey_list = '';
                }
                ...
                $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
                if ($registration_ids) {
                    $survey_list = SurveyModel::where(['land_conversion_status' => $trxn_type])->whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'id')->toArray();
                } else {
                    $survey_list = '';
                }
    ...
    public function deleteConversionSurvey($survey_id, $conversion_id) {
        if ($survey_id && $conversion_id) {
            $delete = LandConversionSurveyModel::where(['survey_id' => $survey_id, 'conversion_id' => $conversion_id])->delete();
            $update_land_conversion_status_in_survey_table = SurveyModel::where(['id' => $survey_id])->update(['land_conversion_status' => 'N']);
        }
        $msg = UtilityModel::getMessage('Land Conversion data deleted successfully!');
        return redirect('transaction/land-conversion/edit/' . $conversion_id)->with('message', $msg);
    }
