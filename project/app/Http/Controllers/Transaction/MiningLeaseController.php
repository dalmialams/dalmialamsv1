        ...
        if ($id) {
            ...
            if (!empty($path_arr) && $file_error == 0) {
                foreach ($path_arr as $key => $value) {
                    if (!empty($value) && isset($posted_docs[$key])) {
    ...
    public function miningLeaseSurvey($village_id = null, $type = null, $trxn_type = 'N', $from_mis = null) {
        ...
        if (!$from_mis) {
            if ($village_id && $type == 'ajax') {
                ...
                $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
                if ($registration_ids) {
                    $survey_list = SurveyModel::where(['mining_applied_status' => $trxn_type, 'purpose' => 'CD00002'])->whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'id')->toArray();
                } else {
                    $survey_list = '';
                }
