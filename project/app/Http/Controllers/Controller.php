...
class Controller extends BaseController
{
    ...
    function __construct()
    {
        ...
        // if (Auth::check()) {
        //     $user = Auth::user();
        //     echo '9999'; echo "<pre>"; print_r($user);exit;
        //     } 
        $this->middleware(function ($request, $next) {
            ...
            if (isset($_REQUEST['reg_uniq_no'])) {
                $unique_id_number = RegistrationModel::where(['id' => $_REQUEST['reg_uniq_no']])->pluck('reg_id')->toArray();
    ...
    public function auth_check_exists()
    {
        if (Auth::check()) {
    ...
    public function getAllStates()
    {
        ...
        if ($this->user_type == 'admin') {
            $states = StateModel::where(['fl_archive' => "N"])->orderBy('state_name')->pluck('state_name', 'id')->toArray();
        } else {
            $assigned_states = $this->assigned_states;
            $assigned_states = explode(",", $assigned_states);
            $state_str = "'" . implode("','", $assigned_states) . "'";
            $states = StateModel::whereRaw("id in ($state_str)")->where(['fl_archive' => "N"])->orderBy('state_name')->pluck('state_name', 'id')->toArray();
            // t($state_str, 1);
        }
    ...
    {
        $codeList = CodeModel::where(['cd_type' => "document_type"])->orderBy('cd_desc', 'ASC')->pluck('cd_desc', 'id')->toArray();
       
        $codes = array_merge(array('' => 'Select'), $codeList);
        return $codes;
    }

    public function getAllPropertyIds()
    {
        $propertyIdList = PropertyModel::where(['fl_archive' => "N"])->orderBy('property_name')->pluck('property_name', 'id')->toArray();
       
        $proerties = array_merge(array('' => 'Select'), $propertyIdList);
        return $proerties;
    }
    ...
    {
        if (!$state_id) {
            $district = DistrictModel::where(['fl_archive' => "N"])->orderBy('district_name')->pluck('district_name', 'id')->toArray();
        } else if ($state_id && $state_number == 'single') {
            $district = DistrictModel::where(['state_id' => "$state_id", 'fl_archive' => "N"])->orderBy('district_name')->pluck('district_name', 'id')->toArray();
        } else if ($state_id && $state_number == 'multiple_state' && $type == null) {
            if ($this->user_type == 'admin') {
                $district = DistrictModel::whereRaw("state_id in ({$state_id}) AND fl_archive = 'N'")->orderBy('state_id')->orderBy('district_name')->pluck('district_name', 'id')->toArray();
            } else {
                ...
                if ($assgined_districts_str) {
                    $district = DistrictModel::whereRaw("id in ($assgined_districts_str) AND fl_archive = 'N'")->orderBy('state_id')->orderBy('district_name')->pluck('district_name', 'id')->toArray();
                } else {
                    $district = '';
                }
            }
    ...
    {
        if (!$district_id) {
            $block = BlockModel::where(['fl_archive' => "N"])->orderBy('block_name')->pluck('block_name', 'id')->toArray();
        } else {
            $block = BlockModel::where(['district_id' => "$district_id", 'fl_archive' => "N"])->orderBy('block_name')->pluck('block_name', 'id')->toArray();
        }
    ...
    {
        if (!$block_id) {
            $villages = VillageModel::orderBy('village_name')->pluck('village_name', 'id')->toArray();
        } else {

            $villages = VillageModel::where(['block_id' => "$block_id"])->orderBy('village_name')->pluck('village_name', 'id')->toArray();
        }
    ...
    {
        if (!$state_id) {
            $city = CityModel::where(['fl_archive' => "N"])->orderBy('city_name')->pluck('city_name', 'id')->toArray();
        } else {
            $city = CityModel::where(['state_id' => "$state_id", 'fl_archive' => "N"])->orderBy('city_name')->pluck('city_name', 'id')->toArray();
        }
    ...
    public function getAllSubClassification($classification_id = '')
    {
        if (!$classification_id) {
            $city = SubClassificationModel::where(['fl_archive' => "N"])->orderBy('sub_name')->pluck('sub_name', 'id')->toArray();
        } else {
            $city = SubClassificationModel::where(['classification_id' => "$classification_id", 'fl_archive' => "N"])->orderBy('sub_name')->pluck('sub_name', 'id')->toArray();
        }
        $city = array_merge(array('' => 'Select'), $city);
        return $city;
    }
    ...
    {
        $codes = [];
        $codes = CodeModel::where(['cd_type' => "$type"])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        // t($codes);exit;
    ...
    public function getAllSurvey($registration_id = null, $village_id = null, $type = null, $list_with_survey_id = false)
    {
        ...
        if ($registration_id) {
            $survey_list = SurveyModel::where(['registration_id' => $registration_id])->orderBy('id')->pluck('survey_no', 'survey_no')->toArray();
            ...
            $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
            if ($registration_ids) {
                $survey_list = SurveyModel::whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'id')->toArray();
            } else {
                $survey_list = '';
            }
            ...
            if ($this->user_type == 'admin') {
                $survey_list = SurveyModel::orderBy('id')->pluck('survey_no', 'survey_no')->toArray();
                if ($list_with_survey_id) {

                    $survey_list = SurveyModel::orderBy('id')->pluck('survey_no', 'id')->toArray();
                    if ($survey_list) {
                        $surveyList[''] = 'Select';
                ...
                if ($this->assigned_states) {
                    ...
                    if ($assgined_districts_str) {
                        ...
                        $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
                        $survey_list = ($registration_ids) ? SurveyModel::whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'survey_no')->toArray() : '';
                        if ($list_with_survey_id) {
                            $survey_list = ($registration_ids) ? SurveyModel::whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'id')->toArray() : '';
                            $survey_list = SurveyModel::orderBy('id')->pluck('survey_no', 'id')->toArray();
                            if ($survey_list) {
                                $surveyList[''] = 'Select';
                                foreach ($survey_list as $key => $value) {
    ...
    public function getAllPatta($village_id = null, $type = null)
    {
        ...
        if ($village_id && $type == 'ajax') {
            ...
            $patta_list = PattaModel::where(['village_id' => $village_id, 'fl_archive' => 'N'])->orderBy('id')->pluck('patta_no', 'id')->toArray();
            $patta_list = array_merge(array('' => 'select'), $patta_list);
            return $patta_list;
            // t($survey_list, 1);
        } else {
            $patta_list = PattaModel::orderBy('id')->pluck('patta_no', 'patta_no')->toArray();
        }
    ...
    public function getAllRegistration($village_id = null, $type = null, $parentConv = null)
    {
        ...
        if ($this->user_type == 'admin') {
            if ($type == 'ajax') {
                ...
                if ($village_id) {
                    $registration_list = RegistrationModel::where(['village_id' => $village_id])->whereRaw($cusCond)->orderBy('id')->pluck('id', 'id')->toArray();
                } else {
                    $registration_list = RegistrationModel::whereRaw($cusCond)->orderBy('id')->pluck('id', 'id')->toArray();
                }
    ...
    public function get_citys_by_state(Request $request)
    {
        $state_id = $request->get('state_id');
        $cities = StateModel::find($state_id)->cities()->select('id as option_code', 'city_name as option_desc')->orderBy('city_name', 'ASC')->get();
        $data['optionList'] = $cities;
        return $data;
    }
}
