...
//      $this->data['purchase_type'] = $this->getCodesDetails('purchase_type');
    ...
    public function view_logs(Request $Request) {
        ...
        $this->data['pageHeading'] = 'Audit <span class="text-danger" > Log </span>';
        $this->data['users'] = $users = DB::table("T_USER")->where(['fl_archive' => 'N'])->get();
    ...
    public function search_logs(Request $Request) {
        ...
        $this->data['data']['jsArr'][] = 'assets/js/formAjaxPlugin.js';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/validation/jquery.validate.js';
        $this->data['pageHeading'] = 'Search <span class="text-danger" > Log </span>';
        $this->data['users'] = $users = DB::table("T_USER")->where(['fl_archive' => 'N'])->get();
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/bootstrap-datepicker/bootstrap-datepicker.js';
        $this->data['include_script_view'] = 'admin.MasterDataManagement.script';
        $postedData = $Request->all();
        ...
        if ($from_date != '' && $to_date != '') {
            ...
            $where .=" and date(created_at) >  CURRENT_DATE - INTERVAL '9 months' ";
        }
        ...
        if (!empty($postedData)) {
            //$where1 = "id ='$user'";
            $results = DB::table("T_MIS_TRANSACTION_HISTORY")->whereRaw($where)->get();
        ...
        return view('admin.MasterDataManagement.searchLog', $this->data);
    }
