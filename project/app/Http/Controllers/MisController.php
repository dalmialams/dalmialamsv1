...
use App\Models\Transaction\MisHistoryModel;
...
use App\Exports\MisRegistrationExport;
use App\Exports\MisMultipleTransactionExport;
...
class MisController extends Controller
{
    ...
    public function registration(Request $request)
    {
        ...
        if ($posted_data) {
            ...
            $insert_mis_history = array();
            $i = 0;
            if (!empty($allSearchData['state_id'])) {
                $transaction_details[$i]['name'] = "state";
                $transaction_details[$i]['value'] = $allSearchData['state_id'];
                $i++;
            }
            if (!empty($allSearchData['district_id'])) {

                $transaction_details[$i]['name'] = "district";
                $transaction_details[$i]['value'] = $allSearchData['district_id'];
                $i++;
            }
            if (!empty($allSearchData['block_id'])) {

                $transaction_details[$i]['name'] = "block";
                $transaction_details[$i]['value'] = $allSearchData['block_id'];
                $i++;
            }

            if (!empty($allSearchData['village_id'])) {

                $transaction_details[$i]['name'] = "village";
                $transaction_details[$i]['value'] = $allSearchData['village_id'];
                $i++;
            }
            if (!empty($allSearchData['sub_registrar'])) {

                $transaction_details[$i]['name'] = "sub_registrar";
                $transaction_details[$i]['value'] = $allSearchData['sub_registrar'];
                $i++;
            }
            if (!empty($allSearchData['regn_no'])) {

                $transaction_details[$i]['name'] = "regn_np";
                $transaction_details[$i]['value'] = $allSearchData['regn_no'];
                $i++;
            }
            if (!empty($allSearchData['id'])) {

                $transaction_details[$i]['name'] = "unique_no";
                $transaction_details[$i]['value'] = $allSearchData['id'];
                $i++;
            }
            if (!empty($allSearchData['purchase_type_id'])) {

                $transaction_details[$i]['name'] = "purchase_type";
                $transaction_details[$i]['value'] = $allSearchData['purchase_type_id'];
                $i++;
            }
            if (!empty($allSearchData['from_date'])) {

                $transaction_details[$i]['name'] = "from_date";
                $transaction_details[$i]['value'] = $allSearchData['from_date'];
                $i++;
            }
            if (!empty($allSearchData['to_date'])) {

                $transaction_details[$i]['name'] = "to_date";
                $transaction_details[$i]['value'] = $allSearchData['to_date'];
                $i++;
            }
            if (!empty($allSearchData['purchaser'])) {

                $transaction_details[$i]['name'] = "purchaser_name";
                $transaction_details[$i]['value'] = $allSearchData['purchaser'];
                $i++;
            }
            if (!empty($allSearchData['legal_entity'])) {

                $transaction_details[$i]['name'] = "legal_entity";
                $transaction_details[$i]['value'] = $allSearchData['legal_entity'];
                $i++;
            }
            if (!empty($allSearchData['purchasing_team_id'])) {

                $transaction_details[$i]['name'] = "purchaser_team";
                $transaction_details[$i]['value'] = $allSearchData['purchasing_team_id'];
                $i++;
            }
            if (!empty($allSearchData['survey_no'])) {

                $transaction_details[$i]['name'] = "survey_no";
                $transaction_details[$i]['value'] = $allSearchData['survey_no'];
                $i++;
            }
            if (!empty($allSearchData['purpose'])) {

                $transaction_details[$i]['name'] = "purpose";
                $transaction_details[$i]['value'] = $allSearchData['purpose'];
                $i++;
            }
            if (!empty($allSearchData['classification'])) {

                $transaction_details[$i]['name'] = "classification";
                $transaction_details[$i]['value'] = $allSearchData['classification'];
                $i++;
            }
            if (!empty($allSearchData['sub_classification'])) {

                $transaction_details[$i]['name'] = "sub_classification";
                $transaction_details[$i]['value'] = $allSearchData['sub_classification'];
                $i++;
            }
            $insert_mis_history['mis_type'] = 'registration';
            $insert_mis_history['created_id'] = $this->data['current_user_id'];
            $insert_mis_history['created_at'] = Carbon::now();
            $insert_mis_history['export_role_id'] = ($this->role_id) ? $this->role_id : 'admin';
            $insert_mis_history['mis_details'] = json_encode($transaction_details);
            $insert_id = MisHistoryModel::insertGetId($insert_mis_history);
            if ($insert_id) {
            ...
            $stateId = isset($posted_data['registration']['state_id']) ? $posted_data['registration']['state_id'] : '';
            $state_name = '';
            if ($stateId != '') {
                $state_name_arr = StateModel::where('id', $stateId)->get();
                $state_name = isset($state_name_arr[0]->state_name) ? $state_name_arr[0]->state_name : '';
            }
            ...
            $reg_id_str = isset($registratiion_ids[0]['id_str']) ? $registratiion_ids[0]['id_str'] : '';

            if (!empty($reg_id_str)) {
                $reg_id_str_array = explode("','", trim($reg_id_str, "'"));
                $survey_number_details = SurveyModel::whereRaw("registration_id in ($reg_id_str) and fl_archive = 'N'")->orderBy('registration_id', 'ASC')->orderBy('survey_no', 'ASC')->get()->toArray();

                $document_details = DocumentModel::whereRaw("registration_id in ($reg_id_str) and fl_archive = 'N'")->orderBy('registration_id', 'ASC')->get()->toArray();

                $payment_details = PaymentModel::whereRaw("registration_id in ($reg_id_str) AND fl_archive = 'N'")->orderBy('registration_id', 'ASC')->get()->toArray();
            ...
            //         'Total Cost', 'State', 'District', 'Block/Taluk', 'Village', 'Source'
            //     ]
            // ];
            $reg_array = [];
            $audit_array = [];
            ...
            if ($registrations) {
                foreach ($registrations as $key => $value) {
                    ...
                    $purchased_area_unit_conv = \App\Models\Common\ConversionModel::where(['code_id' => $value['tot_area_unit']])->value('convers_value_acer');
                    // dd($purchased_area_unit_conv);
                    ...
                    $reg_array[] = [
                        ...
                        $value['vendor'],
                        $purchased_area_unit,
                        number_format((float)$value['tot_area'], 4, '.', ''),
                        number_format((float)($purchased_area_unit_conv * $value['tot_area']), 4, '.', ''),
                        (float) $value['tot_cost'],
                        $state,
                        $district,
                        $block,
                        $village,
            ...
            // $survey_array = [
            //     [
            //         'Reg No', 'Survey No', 'State', 'District', 'Block/Taluk', 'Village', 'Doc Regn No', 'Extend Unit', 'Extend Value', 'Purchased Area', 'Purpose', 'Classification', 'Sub-Classification',
            //         'Mutation Status', 'Encroachment', 'Land Reservation Status', 'Operation Status', 'Dispute Status', 'Mining Status',
            //         'Land Ceiling Status', 'Land Conversion Status', 'Land Exchnage Status', 'Source'
            //     ]
            // ];
            ...
            if ($survey_number_details) {
                foreach ($survey_number_details as $key => $value) {
                    ...
                    $classification = \App\Models\Common\CodeModel::where(['id' => $value['classification'], 'cd_type' => 'plot_classification'])->value('cd_desc');
                    ...
                    $registrations_data = RegistrationModel::where(['id' => $value['registration_id']])->get()->toArray();

                    $state_id = isset($registrations_data[0]['state_id']) ? $registrations_data[0]['state_id'] : '';
                    $district_id = isset($registrations_data[0]['district_id']) ? $registrations_data[0]['district_id'] : '';
                    $block_id = isset($registrations_data[0]['block_id']) ? $registrations_data[0]['block_id'] : '';
                    $village_id = isset($registrations_data[0]['village_id']) ? $registrations_data[0]['village_id'] : '';
                    $doc_reg_no = isset($registrations_data[0]['regn_no']) ? $registrations_data[0]['regn_no'] : '';

                    $state = \App\Models\Common\StateModel::where(['id' => $state_id])->value('state_name');
                    $district = \App\Models\Common\DistrictModel::where(['id' => $district_id])->value('district_name');
                    $block = \App\Models\Common\BlockModel::where(['id' => $block_id])->value('block_name');
                    $village = \App\Models\Common\VillageModel::where(['id' => $village_id])->value('village_name');

                    $survey_source = $value['source'];
                    ...
                    $survey_array[] = [
                        ...
                        $doc_reg_no,
                        $extend_unit,
                        number_format((float)$value['total_area'], 4, '.', ''),
                        number_format((float)$value['purchased_area'], 4, '.', ''),
                        $purpose,
                        $classification,
                        $sub_classification,
                        $mutation_status,
                        $encroachment,
                        $land_reservation_status,
                        $operation_status,
                        $dispute_status,
                        $mining_status,
                        $ceiling_status,
                    ...
                    $survey_details_array[] = $value['registration_id'];
                }
            }

            // survey no   if no survey against that reg id

            if (!empty($survey_details_array)) {
                foreach ($reg_id_str_array as $key => $val) {
                    if (!in_array($val, $survey_details_array))
                        $survey_array[] = [
                            $val,
                            ' ',
                            ' ',
            ...
            } else if (!empty($reg_id_str_array)) {
                foreach ($reg_id_str_array as $key => $val) {
                    $survey_array[] = [
                        $val,
                        ' ',
            ...
            if ($document_details) {
                foreach ($document_details as $key => $value) {
                    ...
                    $document_reg_array[] = $value['registration_id'];
                }
            }

            if (!empty($document_reg_array)) {
                foreach ($reg_id_str_array as $key => $val) {
                    if (!in_array($val, $document_reg_array))
                        $document_array[] = [
                            $val,
                            ' ',
                            ' ',
            ...
            // payment details   if no reg against that reg id
            ...
            $file_name = 'MIS-Registration-' . $state_name . '-' . date('dmY');
            set_time_limit(0);
            ini_set('memory_limit', '1G');

            $excel_data = ['reg_arr' => $reg_array, 'survey_arr' => $survey_array, 'document_arr' => $document_array, 'payment_arr' =>  $payment_array, 'audit_arr' => $audit_array];

            // dd($excel_data);
            return Excel::download(new MisRegistrationExport($excel_data), $file_name . '.xlsx');

            // Excel::create($file_name, function ($excel) use ($reg_array, $survey_array, $document_array, $payment_array, $audit_array) {
            // return Excel::download(function ($excel) use ($reg_array, $survey_array, $document_array, $payment_array, $audit_array) {
    ...
    public function transaction(Request $request)
    {
        ...
        if ($posted_data) {
            ...
            $i = 0;
            $allSearchData = $posted_data['registration'];
            $allSearchData1 = isset($posted_data['coversion_parent']) ? $posted_data['coversion_parent'] : '';
            $selected_transaction = isset($allSearchData['trxn']) ? $allSearchData['trxn'] : '';
            if (!empty($allSearchData['trxn'])) {
                $transaction_details[$i]['name'] = "Transactions";
                $transaction_details[$i]['value'] = $allSearchData['trxn'];
                $i++;
            }
            ...
            if (!empty($allSearchData['survey_id'])) {
                $transaction_details[$i]['name'] = "survey_no";
                $transaction_details[$i]['value'] = $allSearchData['survey_id'];
                $i++;
            }
            if (!empty($allSearchData1['registration_id'])) {
                $transaction_details[$i]['name'] = "registration_no";
                $transaction_details[$i]['value'] = $allSearchData1['registration_id'];
                $i++;
            }
            if (!empty($allSearchData1['date_from'])) {
                $transaction_details[$i]['name'] = "from_date";
                $transaction_details[$i]['value'] = $allSearchData1['date_from'];
                $i++;
            }
            if (!empty($allSearchData1['date_to'])) {
                $transaction_details[$i]['name'] = "to_date";
                $transaction_details[$i]['value'] = $allSearchData1['date_to'];
                $i++;
            }
            $insert_mis_history['mis_type'] = 'transaction';
            $insert_mis_history['created_id'] = $this->data['current_user_id'];
            $insert_mis_history['created_at'] = Carbon::now();
            $insert_mis_history['export_role_id'] = ($this->role_id) ? $this->role_id : 'admin';
            $insert_mis_history['mis_details'] = json_encode($transaction_details);
            $insert_id = MisHistoryModel::insertGetId($insert_mis_history);
            if ($insert_id) {
                $msg = UtilityModel::getMessage('Transaction data saved successfully!');
            }
            $registratiion_ids = RegistrationModel::selectRaw("array_to_string(array_agg(distinct QUOTE_LITERAL(id)), ',') as id_str")->whereRaw($cond)->get()->toArray();
            ...
            // $dispute_array = [
            //     [
            //         'Transaction ID', 'Registration NO', 'Survey No.', 'Date', 'Reminder Date', 'Next Hearing Date', 'Litigation Type', 'Remarks',
                ...
                foreach ($dispute_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            //get reg no
                            $survey_id  = isset($s_value['survey_id']) ? $value['survey_id'] : '';
                            $reg_id = '';
                            if ($survey_id != '') {
                                $reg_arr = SurveyModel::join('T_REGISTRATION', 'T_REGISTRATION.id', '=', 'T_SURVEY.registration_id', 'left')->get()->toArray();
                                $reg_id = isset($reg_arr[0]['registration_id']) ? $reg_arr[0]['registration_id'] : '';
                            }
            ...
            if ($land_ceiling_details) {
                foreach ($land_ceiling_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            $survey_id  = isset($s_value['survey_id']) ? $s_value['survey_id'] : '';
                            $reg_id = '';
                            if ($survey_id != '') {
                                $reg_arr = SurveyModel::join('T_REGISTRATION', 'T_REGISTRATION.id', '=', 'T_SURVEY.registration_id', 'left')->get()->toArray();
                                $reg_id = isset($reg_arr[0]['registration_id']) ? $reg_arr[0]['registration_id'] : '';
            ...
            // $inspection_array = [
            //     [
            //         'Transaction ID', 'Registration No', 'Survey No.', 'Transaction Date', 'Inspection Date', 'Encroachment', 'Encroachment Type', 'Remarks',
            //         'State', 'District', 'Block/Taluk', 'Village'
            //     ]
            // ];
            ...
            if ($land_inspection_details) {
                foreach ($land_inspection_details as $key => $value) {
                    ...
                    //get reg details 
                    $survey_id  = isset($value['survey_id']) ? $value['survey_id'] : '';
                    $reg_id = '';
                    if ($survey_id != '') {
                        $reg_arr = SurveyModel::join('T_REGISTRATION', 'T_REGISTRATION.id', '=', 'T_SURVEY.registration_id', 'left')->get()->toArray();
                        $reg_id = isset($reg_arr[0]['registration_id']) ? $reg_arr[0]['registration_id'] : '';
            ...
            if ($lease_details) {
                foreach ($lease_details as $key => $value) {
                    ...
                    $lease_array[] = [
                        ...
                        $value['lease_name'],
                        $value['lessor_name'],
                        (float) $value['lease_monthly_amount'],
                        $lease_date_range,
                        $value['percentage_escalation'],
                        $status
                    ];
                }
            }
                ...
                foreach ($parent_details as $key => $value) {
                    ...
                    if ($registration_res) {
                        foreach ($registration_res as $r_key => $r_value) {
                            ...
                            $parent_array[] = [
                                $value['id'],
                                $value['converted_registration_id'],
                                $transaction_date,
                                $value['old_registration_id'],
                                $reg_id,
                                $purchaser_name,
                                $legal_entity,
                                $sub_registrar,
                                (float) $purchased_area,
                                (float) $cost,
                                $state,
                                $district,
                                $block,
            ...
            $file_name = 'MIS-TRANSACTION' . $state_name . '-' . date('dmY');
            if ($selected_transaction == 'all') {

                $file_name = 'MIS-TRANSACTION-ALL_' . $state_name . '-' . date('dmY');

                $excel_data = ['dispute_arr' => $dispute_array, 'ceiling_arr' => $ceiling_array, 'conversion_arr' => $conversion_array, 'exchange_arr' =>  $exchange_array, 'inspection_arr' => $inspection_array, 'reservation_arr' => $reservation_array, 'lease_arr' => $lease_array, 'mining_arr' => $mining_array, 'operation_arr' => $operation_array, 'payment_arr' => $payment_array, 'mutation_arr' => $mutation_array, 'hypo_arr' => $hypothetication_array, 'parent_arr' => $parent_array];

                return Excel::download(new MisMultipleTransactionExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'hypothecation') {

                $file_name = 'MIS_TRANSACTION_HYPOTHECATION_' . $state_name . '-' . date('dmY');

                $excel_data = ['hypo_arr' => $hypothetication_array, 'type' => 'single'];

                return Excel::download(new HypotheticationExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'disputes') {

                $file_name = 'MIS_TRANSACTION_DISPUTES_' . $state_name . '-' . date('dmY');
                $excel_data = ['dispute_arr' => $dispute_array, 'type' => 'single'];
                return Excel::download(new DisputeExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'land_ceiling') {

                $file_name = 'MIS_TRANSACTION_CEILING_' . $state_name . '-' . date('dmY');

                $excel_data = ['ceiling_arr' => $ceiling_array, 'type' => 'single'];
                return Excel::download(new CeilingExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'land_conversion') {

                $file_name = 'MIS_TRANSACTION_CONVERSION_' . $state_name . '-' . date('dmY');

                $excel_data = ['conversion_arr' => $conversion_array, 'type' => 'single'];
                return Excel::download(new ConversionExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'land_exchange') {

                $file_name = 'MIS_TRANSACTION_EXCHANGE_' . $state_name . '-' . date('dmY');

                $excel_data = ['exchange_arr' => $exchange_array, 'type' => 'single'];
                return Excel::download(new ExchangeExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'land_inspection') {

                $file_name = 'MIS_TRANSACTION_INSPECTION_' . $state_name . '-' . date('dmY');

                $excel_data = ['inspection_arr' => $inspection_array, 'type' => 'single'];
                return Excel::download(new InspectionExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'land_reservation') {

                $file_name = 'MIS_TRANSACTION_RESERVATION_' . $state_name . '-' . date('dmY');

                $excel_data = ['reservation_arr' => $reservation_array, 'type' => 'single'];
                return Excel::download(new ReservationExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'payment') {

                $file_name = 'MIS_TRANSACTION_PAYMENT_' . $state_name . '-' . date('dmY');

                $excel_data = ['payment_arr' => $payment_array, 'type' => 'single'];
                return Excel::download(new PaymentExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'under_operation') {

                $file_name = 'MIS_TRANSACTION_UNDER_OPERATION_' . $state_name . '-' . date('dmY');

                $excel_data = ['operation_arr' => $operation_array, 'type' => 'single'];
                return Excel::download(new OperationExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'mutation') {

                $file_name = 'MIS_TRANSACTION_MUTATION_' . $state_name . '-' . date('dmY');

                $excel_data = ['mutation_arr' => $mutation_array, 'type' => 'single'];
                return Excel::download(new MutationExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'mining_lease') {

                $file_name = 'MIS_TRANSACTION_MINING_LEASE_' . $state_name . '-' . date('dmY');

                $excel_data = ['mining_arr' => $mining_array, 'type' => 'single'];
                return Excel::download(new MiningExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'lease') {

                $file_name = 'MIS_TRANSACTION_LEASE_' . $state_name . '-' . date('dmY');

                $excel_data = ['lease_arr' => $lease_array, 'type' => 'single'];
                return Excel::download(new LeaseExport($excel_data), $file_name . '.xls');
            } else if ($selected_transaction == 'coversion_parent') {

                $file_name = 'MIS_TRANSACTION_CONVERSION_TO_PARENT' . $state_name . '-' . date('dmY');

                $excel_data = ['parent_arr' => $parent_array, 'type' => 'single'];
                return Excel::download(new ParentExport($excel_data), $file_name . '.xls');
            }
        }
        $this->data['dataPresent'] = 'yes';
    ...
    // public function land_inspection(Request $request)
    // {
    //     $this->data['pageHeading'] = 'Inspection <span class="text-danger" >Report</span>';
    ...
    //     return view('admin.mis.land_inspection', $this->data);
    // }
    ...
    public function land_inspection(Request $request)
    {
        ...
        $this->data['data']['jsArr'][] = 'assets/plugins/tables/datatables/dataTables.responsive.js';
        $this->data['data']['jsArr'][] = 'assets/js/pages/tables-data.js';
        $this->data['include_script_view'] = 'admin.mis.land_inspection_script';
        //        $this->data['data']['jsArr'][] = 'assets/js/autoNumeric-min.js';
        $this->data['data']['jsArr'][] = 'assets/js/formAjaxPlugin.js';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/select2/select2.js';
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/bootstrap-datepicker/bootstrap-datepicker.js';
        ...
        if (!empty($posted_data)) {
            $cond = " 1=1 AND t_ins.fl_archive ='N' ";

            if(!empty($posted_data['inspection']['state_id'])){
                $cond .= " AND t_ins.state_id = '".$posted_data['inspection']['state_id']."'";
            }

            if(!empty($posted_data['inspection']['district_id'])){
                $cond .= " AND t_ins.district_id = '".$posted_data['inspection']['district_id']."'";
            }

            if(!empty($posted_data['inspection']['block_id'])){
                $cond .= " AND t_ins.block_id = '".$posted_data['inspection']['block_id']."'";
            }

            if(!empty($posted_data['inspection']['village_id'])){
                $cond .= " AND t_ins.village_id = '".$posted_data['inspection']['village_id']."'";
            }

            if(!empty($posted_data['inspection']['survey_id'])){
                $cond .= " AND t_ins.survey_id = '".$posted_data['inspection']['survey_id']."'";
            }

            if(!empty($posted_data['inspection']['encroachment'])){
                $cond .= " AND t_ins.encroachment = '".$posted_data['inspection']['encroachment']."'";
            }

            if(!empty($posted_data['inspection']['encroachment_type'])){
                $cond .= " AND t_ins.encroachment_type = '".$posted_data['inspection']['encroachment_type']."'";
            }
            ...
            $sql = 'SELECT 
                ...
                survey_no,
                t_ins.encroachment,
                t_ins.encroachment_type,
                cd_desc,
                t_ins.insp_date,
                t_ins.description
            FROM "T_INSPECTION" t_ins 
            LEFT JOIN "T_STATE" tst ON tst.id = t_ins.state_id
            LEFT JOIN "T_DISTRICT" tdt ON tdt.id = t_ins.district_id
            LEFT JOIN "T_BLOCK" tbl ON tbl.id = t_ins.block_id
            LEFT JOIN "T_VILLAGE" tvl ON tvl.id = t_ins.village_id
            LEFT JOIN "T_SURVEY" tsv ON tsv.id = t_ins.survey_id
            LEFT JOIN "T_CODES" tcd ON tcd.id = t_ins.encroachment_type
            WHERE ' . $cond;

            $res = DB::select($sql);

            $newArray = array();
            $newArray[] = ["Inspection ID", "State", "District", "Block", "Village", "Survey Number", "Date", "Encroachment", "Encroachment Type", "Remarks"];
            ...
            return Excel::download(new class($newArray) implements FromArray {
                protected $newArray;
    ...
    public function mortgage_land(Request $request)
    {
        $this->data['pageHeading'] = 'Mortgage Land <span class="text-danger" >Add</span>';
        ...
        $this->data['data']['jsArr'][] = 'assets/plugins/forms/bootstrap-filestyle/bootstrap-filestyle.js';
        $this->data['include_script_view'] = 'admin.mis.mortgage_land_script';

        $posted_data = $request->all();
        
        if (!empty($posted_data)) {
            $cond = " 1=1 AND hyp.fl_archive = 'N' ";
            $reg_where = " 1=1 ";

            if(!empty($posted_data['hypothecate']['hyp_name'])){
                $cond .= " AND hyp.hyp_name = '".$posted_data['hypothecate']['hyp_name']."'";
            }

            if(!empty($posted_data['hypothecate']['trxn_type'])){
                $cond .= " AND hyp.trxn_type = '".$posted_data['hypothecate']['trxn_type']."'";
            }

            if(!empty($posted_data['hypothecate']['hyp_date'])){
                $cond .= " AND TO_CHAR(hyp_date,'FMdd/FMMM/YYYY') = '".$posted_data['hypothecate']['hyp_date']."'";
            }

            if(!empty($posted_data['hypothecate']['hyp_with'])){
                $cond .= " AND hyp.hyp_with = '".$posted_data['hypothecate']['hyp_with']."'";
            }

            if(!empty($posted_data['registration_id'])){
                $reg_where .= " AND registration_id IN ('".implode('\',\'',$posted_data['registration_id'])."')";
            }
            ...
            $sql = 'SELECT 
                        hyp.id,
                        hyp.hyp_name as hyp_name_code,
                        tcd_hn.cd_desc as hyp_name,
                        hyp.trxn_type,
                        hyp.value,
                        hyp.hyp_with as hyp_with_code,
                        tcd_wt.cd_desc as hyp_with_name,
                        hyp_r.registration_ids,
                        TO_CHAR(hyp_date,\'FMdd/FMMM/YYYY\') as hyp_date,
                        hyp.value,
	                    hyp.description as remarks
                    FROM public."T_HYPOTHECATION" hyp
                    JOIN (
                        SELECT 
                            hypothecate_id, 
                            string_agg(registration_id, \',\') as registration_ids 
                        FROM 
                            public."T_HYPOTHECATION_REGISTRATION" 
                        WHERE
                            '.$reg_where.'
                        GROUP BY hypothecate_id
                    ) hyp_r ON hyp_r.hypothecate_id = hyp.id
                    LEFT JOIN "T_CODES" tcd_hn ON tcd_hn.id = hyp.hyp_name
                    LEFT JOIN "T_CODES" tcd_wt ON tcd_wt.id = hyp.hyp_with
            ...
            $newArray = array();
            $newArray[] = ["Mortgage Land Number", "Mortgage Land Name", "Transaction Type", "Registration Number's", "Mortgage Land With", "Date", "Value", "Remarks"];
            ...
            if (!empty($res)) {
                for ($i = 0; $i < count($res); $i++) {
                    ...
                    $newArray[] = [
                        $row->id,
                        $row->hyp_name,
                        ($row->trxn_type == 'Y') ? 'Hypothicate' : 'Release',
        ...
        // dd($this->data);
        return view('admin.mis.mortgage_land', $this->data);
    }
    ...
    public function common(Request $request, $option = true, $json = true)
    {
        ...
        if ($transaction_type == 'hypothecation') {
            ...
            $old_patta_no = \App\Models\LandDetailsManagement\PattaModel::where(['fl_archive' => 'N'])->whereRaw("mutation_status = 'Y' or mutation_status = 'P'")->orderBy('patta_no')->pluck('patta_no', 'id')->toArray();
    ...
    public function test_mail(Request $requests)
    {
        SendMailModel::passwordEmailer("http://hello.com", "argha.paul@cyberswift.com");
    }
}
