...
class MapController extends Controller {
    ...
    public function map(Request $request) {
		...
		$geo_url = 'lmsdev.dalmiabharat.com';
	...
	{	
       ...
       // $plotDetails = SurveyModel::selectRaw("st_contains(\"T_SURVEY_MAP\".wkb_geometry,ST_GeomFromText('POINT(" . $coordinate[0] . " " . $coordinate[1] . ")',4326))");
			...
			$servey_data = DB::table('T_SURVEY')
				->leftJoin('T_SURVEY_MAP', 'T_SURVEY.id', '=', 'T_SURVEY_MAP.survey_id')
				->where('T_SURVEY_MAP.survey_id', '=', $plotDetails[0]['survey_id'])
				->get();
			...
			$village_name = isset($village_result[0]['village_name'])? $village_result[0]['village_name'] : '';
                ...
                if (!empty($plotDetails)) {
                   ...
                   // $PLS_PLOT_TYPE = $this->all_function->get_dynamic_dropdown('plot_type', $PLS_PLOT_TYPE, true);
				    $extent = DB::select("Select ST_X(ST_Centroid(ST_Transform(wkb_geometry, 4326))) AS long, ST_Y(ST_Centroid(ST_Transform(wkb_geometry, 4326))) AS lat FROM \"T_SURVEY_MAP\" where survey_no ='".$plotDetails[0]['survey_no']."'");
	...
	{	
		...
		$survey_result = DB::table('T_SURVEY')
                    ->join('T_SURVEY_MAP', 'T_SURVEY.id', '=', 'T_SURVEY_MAP.survey_id')
                    ->where('T_SURVEY.village_id', '=', $village_id)
					->where('T_SURVEY.fl_archive', '=', 'N')
                    ->select('T_SURVEY.*', 'T_SURVEY_MAP.*')
                    ->get();
	...
	public function get_serveyno_json(Request $request)
	{
...
//$survey_id = '102/1';		
	...
	public function getStatePoint(Request $request) {
	...
	public function getDistrictPoint(Request $request) {
		   ...
		   $string = '';
			   $data = collect($query)->map(function($x){ return (array) $x; })->toArray(); 
					...
					foreach ($data as $key => $value) {
						...
						$servey_data = DB::table('T_SURVEY')
						->leftJoin('T_SURVEY_MAP', 'T_SURVEY.id', '=', 'T_SURVEY_MAP.survey_id')
						->where('T_SURVEY_MAP.survey_id', '=', $value['survey_id'])
						->get();
						...
						$extent = DB::select("Select ST_X(ST_Centroid(ST_Transform(wkb_geometry, 4326))) AS long, ST_Y(ST_Centroid(ST_Transform(wkb_geometry, 4326))) AS lat FROM \"T_SURVEY_MAP\" where survey_no ='".$value['survey_no']."'");
						$longitude = $extent[0]->long;
						$latitude = $extent[0]->lat;
						}else{
							$longitude = '';
