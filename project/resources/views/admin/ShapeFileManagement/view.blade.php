...
<a style="float:right" class="btn btn-primary" href="<?= url('document-management/document/shape-file-list') ?>">Shape Zip List</a>
</div>
            ...
            <div class="col-lg-12">
                ...
                <div class="panel panel-primary  toggle panelMove panelClose panelRefresh">
                    ...
                    <div class="panel-body">
					 <p class="text-center"><span class="red-sign">*</span> Red color does not have matched value with the master data. please verify!</p>
						...
						$get_district_value = array_column($shpTables->district_master, 'district_name');
						$get_state_value = array_column($shpTables->state_master, 'state_name');
						$get_taluk_value = array_column($shpTables->taluk_master, 'block_name');
                        $get_village_value = array_column($shpTables->village_master, 'village_name');

                            if(!in_array(($value->district), $get_district_value)){
                               $not_matched = 'not-matched';
							}else if(!in_array(($value->state), $get_state_value)){
                               $not_matched = 'not-matched';
							}else if(!in_array(($value->taluk), $get_taluk_value)){
                               $not_matched = 'not-matched';
							}else if(!in_array(($value->village), $get_village_value)){
                               $not_matched = 'not-matched';
							}else{
                               $not_matched = '';
							}														
						?>
							<tr class="<?= $not_matched ?>">
	...
	<a class="btn btn-success" onclick="return confirm('Are you sure to import shape file?')" href="<?= url('document-management/document/importShp/'.$table_name.'/survey_plot/'.$data_id) ?>"><i class="fa fa-external-link-square" aria-hidden="true"></i> Import Shape File</a>
	
	<a class="btn btn-info" onclick="return confirm('Are you sure to export shape file?')" href="<?= url('document-management/document/data-export-shp/'.$table_name.'/survey_plot/'.$data_id) ?>"><i class="fa fa-external-link-square" aria-hidden="true"></i> Export Shape File</a>
...
var parcel_boundry_url = '<?php echo URL("document-management/document/shape-map-view/survey_plot/");?>/<?= $table_name ?>';
