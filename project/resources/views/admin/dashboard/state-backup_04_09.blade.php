    ...
    <table id="tabletools1" class="table table-striped table-bordered" cellspacing="0" width="100%">
        <thead>
            <tr>
                ...
                <th  class="text-center">Leased Area (Acre)</th>
                <th  class="text-center">Leased Area (Hectare)</th>
        ...
        <tbody>
            ...
            <?php
                if(!empty($states)){
                    $totalPurchasedAreaAcre = 0;
                    $totalPurchasedAreaHector = 0;
                    $totalLeasedAreaAcre = 0;
                    $totalLeasedAreaHector = 0;
                    $totalCost = 0;
                    $allStateIds = '1';
                    ...
                    foreach($states as $key => $value){
                        ...
                        $totalPurchasedAreaAcre += $value['purchased_area_in_acre'];
                        $totalPurchasedAreaHector += $value['purchased_area_in_hector'];
                        $totalLeasedAreaAcre += $value['leased_area_in_acre'];
                        $totalLeasedAreaHector += $value['leased_area_in_hector'];
                        $totalCost += $value['total_cost'];

                        $allStateIds = $allStateIds.','.$value['state_id'];

                        ?>
                            <tr>
                                <td  class="text-left">
                                    <?php if ($filter == '') { ?>
                                        <span class="context-menu-one btn btn-neutral st-level" tag="<?= $value['state_id']; ?>"> <?php echo $value['state_name']; ?></span>
                                    <?php } elseif ($filter == 'District') { ?>
                                        {{-- <a href="<?= url('dashboard/district?state_id=' . $value['state_id'] . '&filter=District'); ?>" class="context-menu-one btn btn-neutral" tag="<?= $value['state_id']; ?>"> --}}
                                            <span class="context-menu-one btn btn-neutral st-level" tag="<?= $value['state_id']; ?>"> <?php echo $value['state_name']; ?></span>
                                        {{-- </a> --}}
                                    <?php } elseif ($filter == 'Block/Taluk') { ?>
                                        {{-- <a href="<?= url('dashboard/block?state_id=' . $value['state_id'] . '&filter=Block/Taluk'); ?>" class="context-menu-one btn btn-neutral" tag="<?= $value['state_id']; ?>"> --}}
                                            <span class="context-menu-one btn btn-neutral st-level" tag="<?= $value['state_id']; ?>"> <?php echo $value['state_name']; ?></span>
                                        {{-- </a> --}}
                                    <?php } elseif ($filter == 'Village') { ?>
                                        {{-- <a href="<?= url('dashboard/village?state_id=' . $value['state_id'] . '&filter=Village'); ?>" class="context-menu-one btn btn-neutral" tag="<?= $value['state_id']; ?>"> --}}
                                            <span class="context-menu-one btn btn-neutral st-level" tag="<?= $value['state_id']; ?>"> <?php echo $value['state_name']; ?></span>
                                        {{-- </a> --}}
                                    <?php } else { ?>
                                        {{-- <a href="<?= url('dashboard/details?state_id=' . $value['state_id'] . '&filter=' . $filter); ?>" class="context-menu-one btn btn-neutral" tag="<?= $value['state_id']; ?>"> --}}
                                            <span class="context-menu-one btn btn-neutral st-level" tag="<?= $value['state_id']; ?>"> <?php echo $value['state_name']; ?></span>
                                        {{-- </a> --}}
                                    <?php } ?>
                                </td>
                                <td class="all_total_area" style="text-align: right;"><?= $value['purchased_area_in_acre']; ?></td>
                                <td class="all_total_area" style="text-align: right;"><?= $value['purchased_area_in_hector']; ?></td>

                                <td class="all_total_area" style="text-align: right;"><?= $value['leased_area_in_acre']; ?></td>
                                <td class="all_total_area" style="text-align: right;"><?= $value['leased_area_in_hector']; ?></td>

                                <td style="text-align: right;" >
                                    <i class="fa fa-spinner fa-spin" id="<?= $value['state_id']; ?>" style="font-size:24px;visibility: hidden"></i> &nbsp;
                                    <span class="all_total_cost custom_underline" onmouseover ="totalCostDistribution('<?= $value['state_id'];?>','','','','<?= $value['state_id'];?>')"><?= $value['total_cost']; ?></span>
                                </td>
                            </tr>
                        <?php
                    }
        ...
        <tfoot>
            <tr>
                ...
                <th class="all_total_area" style="text-align: right;"><?= $totalPurchasedAreaAcre ?></th>
                <th class="all_total_area" style="text-align: right;"><?= $totalPurchasedAreaHector ?></th>
                <th class="all_total_area" style="text-align: right;"><?= $totalLeasedAreaAcre ?></th>
                <th class="all_total_area" style="text-align: right;"><?= $totalLeasedAreaHector ?></th>
                <th style="text-align: right;">  
                    <i class="fa fa-spinner fa-spin" id="total" style="font-size:24px;visibility: hidden"></i>&nbsp;
                    <span class="all_total_cost"><?= $totalCost ?></span>
                </th>
            </tr>
        </tfoot>
    </table>
...
<div id="map_data_view"  style="display:block">
		...
		<script src="{{url('assets/plugins/forms/select2/select2.js')}}" type="text/javascript"></script>
        <script>
 ...
 // palnt location
	...
	visible: true
		...
		var map = new ol.Map({
			...
			layers: [
				bingLayer,state_layer,plant_layer
			],
			target: 'map',
			view: new ol.View({
				maxZoom: 19,
...
<script src="{{url('assets/js/measure.js')}}" type="text/javascript"></script>
