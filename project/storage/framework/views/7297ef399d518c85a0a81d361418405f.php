<div class="row">
    <?php 
    //  function array_value_replace_data($value) {

    //     return ucwords(str_replace('_', ' ', $value));
    // }

    if (!function_exists('array_value_replace_data')) {
        function array_value_replace_data($value){
            return ucwords(str_replace('_', ' ', $value));
        }
      }
    ...
    <div class=" panel-primary ">
        ...
        <div class="panel-body">
            <?php echo Form::open(['url' => url('master/data/submit-data'),'class' => 'form-horizontal reg-form','method' => 'POST','id'=>'my-form', 'enctype' => 'multipart/form-data','role' => 'form']); ?>
                ...
                <div class="row ">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form-group">
                        ...
                        <div class="col-lg-10 col-md-9">
                            ...
                            $masterType = App\Models\Common\CodeModel::where(['cd_type' => "masterdata_type", 'cd_fl_archive' => 'N'])->where('id', '<>', 'CD00156')->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();

                            //echo "<pre>";print_r($masterType);exit;

                            $cdList = App\Models\Common\CodeModel::where('cd_type', '<>', 'masterdata_type')->groupBy('cd_type')->orderBy('cd_type', 'asc')->pluck('cd_type', 'cd_type')->toArray();

                           // echo "<pre>";print_r($cdList);exit;
                            

                        //    public static function array_value_replace_data($value) {

                        //         return ucwords(str_replace('_', ' ', $value));
                        //     }
                            // $cdList = array_map("array_value_replace_data", $cdList);
                                    ...
                                    foreach ($masterType as $key => $value) {
                                        switch ($key) {
                                            ...
                                            case 'document_type':
                                                ...
                                                break;
                                            case 'property_id':
                                                if ((\App\Models\UtilityModel::ifHasPermission('master_property_access', $current_user_id))) {
                                                    $filtrered_master_type[$key] = $value;
                                                }
                                                break;
...
//                              $masterType= App\Models\Common\CodeModel::where(['cd_fl_archive'=>'N'])->select('cd_type','cd_desc','id')->groupBy('cd_type','cd_desc','id')->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
                            ?>
                            <?php echo e(Form::select('data[data]',$filtrered_master_type , isset($data) ? $data : '', ['class'=>'form-control select2 select2-minimum required','onChange'=>'form.submit();'])); ?>
...
</div><?php /**PATH C:\wamp64\www\dalmia_lams\resources\views/admin/MasterDataManagement/nav.blade.php ENDPATH**/ ?>