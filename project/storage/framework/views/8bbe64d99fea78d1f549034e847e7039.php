...
<script>
    ...
    $(function () {
        ...
        $('.all_total_area_hector').autoNumeric('init', {mDec: 3, dGroup: 2});
    ...
    col.each(function () {
          $(this).attr('width', 25);
				  ...
				  /*var boldCols = ['A', 'B', 'C', 'D', 'E'];        
        for ( i=0; i < boldCols.length; i++ ) {
            $('row:eq(-2) c[r^='+boldCols[i]+']', sheet).attr( 's', '2' );  //bold
			 $('row:eq(-2) c[r^='+boldCols[i]+']', sheet).attr( 's', '50' );  //bold
        }*/
				//$('c[r=B3]', sheet).attr('s', '2');
				
				
//$('row:eq(2) c', sheet).attr( 's', '2' );
                // Use the new style on "Age" column
				//$('row:not(:eq(1)) c[r^=A]', sheet).attr('s', '50');
				$('row:not(:eq(1)) c[r^=B]', sheet).attr('s', styleNum);
                $('row:not(:eq(1)) c[r^=C]', sheet).attr('s', styleNum);
                ///$('row:not(:eq(1)) c[r^=D]', sheet).attr('s', styleNum);
                //$('row:not(:eq(1)) c[r^=E]', sheet).attr('s', styleNum);

				
				$('row c[r*="3"]', sheet).attr( 's', '2' );
				//$('row c[r*="13"]', sheet).attr( 's', '52' );
				//$('row c[r*="13"]', sheet).attr( 's', '52' );
				$('c[r=A13]', sheet).attr('s', '50');
				$('c[r=B13]', sheet).attr('s', '52');
				$('c[r=C13]', sheet).attr('s', '52');
				$('c[r=D13]', sheet).attr('s', '52');
				$('c[r=E13]', sheet).attr('s', '52');
				//$('row:first c', sheet).attr('s', '2'); // overwrite first row, so it keeps bold
				//$('row:first c', sheet).attr('s', '50'); // overwrite first row, so it keeps bold
                     ...
                     doc['footer']=(function(page,pages) {
                        return {
                          columns: [
                            {
                              alignment: 'left',
                              text: ['Creato il: ', { text: jsDate.toString() }]
                            },
    ...
    $(function () {
        $.contextMenu({
            ...
            callback: function (key, options) {
                ...
                var state_id = "<?php echo e(Request::get('state_id')); ?>";
                var district_id = "<?php echo e(Request::get('district_id')); ?>";
                var block_id = "<?php echo e(Request::get('block_id')); ?>";
                var village_id = "<?php echo e(Request::get('village_id')); ?>";
                var filter = "<?php echo e(Request::get('filter')); ?>";
...
<?php /**PATH C:\wamp64\www\dalmialamsv1\resources\views/admin/dashboard/script.blade.php ENDPATH**/ ?>