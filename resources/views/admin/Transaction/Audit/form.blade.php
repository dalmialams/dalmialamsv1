...
<?php if (!$viewMode) { ?>
    <div class="panel panel-primary  toggle panelMove panelClose panelRefresh">
        ...
        <div class="panel-body">
            {!! Form::open(['url' => url('transaction/audit/submit-data'),'class' => 'form-horizontal lease-form','method' => 'POST', 'enctype' => 'multipart/form-data','role' => 'form']) !!}
                                ...
                                <div class="col-sm-6 col-xs-12 form-group pull-left">
                                    ...
                                    <div class="col-lg-7 col-md-9 registrationList">
                                        <?php if (isset($audit_data[0]['reg_id'])) { ?>
                                            {{Form::select('', isset($reg_id_arr) ?$reg_id_arr : array('' => 'Select') , isset($audit_data[0]['reg_id']) ? $audit_data[0]['reg_id'] : '',array('class'=>'form-control select2-minimum','disabled' => true))}}
                                            <input type="hidden" name="auditstatusreg[reg_no]" value="<?= $audit_data[0]['reg_id'] ?>" id="uniqueregistrationNumber">
                                        <?php } else { ?>
                                            {{Form::select('auditstatusreg[reg_no]', isset($reg_id_arr) ?$reg_id_arr : array('' => 'Select') , isset($audit_data['reg_id']) ? $audit_data['reg_id'] : '',array('class'=>'form-control select2-minimum','onchange'=>'checkExists($(this).val())','id'=>'uniqueregistrationNumber'))}}
                                        <?php } ?>
                                    </div>
                                    <?php if (!isset($audit_data[0]['reg_id'])) { ?>
                                        <div>
                                            <span class="logo" data-toggle="modal" data-target="#auditstatushelp">
                                                <img src="{{ URL::asset('assets/img/help_icon.png') }}" alt="Dalmia Lams">
                                            </span>
                                        </div>
                                    <?php } ?>
                                    ...
                                    foreach ($masterData as $key => $value) {
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <?php } else { ?>
                                                    <div class="input-group"> <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                                                        {{ Form::text('auditstatus['.$masterid.'][audit_date]', isset($audit_data['data']["regn_date"]) ? $reg_data["regn_date"] : date('d/m/Y'), array("class"=>"form-control required basic-datepicker","placeholder" => "Date")) }}
                                                    </div>
                                                <?php } ?>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <?php } else { ?>
                                                    {{Form::select('auditstatus['.$masterid.'][verified]', isset($type) ?$type : '' , '' ,array('class'=>'form-control select2-minimum'))}}
                                                <?php } ?>
                                            ...
                                            <td class="text-center"> 
                                                ...
                                                <?php } else { ?>
                                                    {{ Form::text('auditstatus['.$masterid.'][remarks]', isset($value["remarks"]) ? $value["remarks"] : "", array("class"=>"form-control")) }}
                                                <?php } ?>
