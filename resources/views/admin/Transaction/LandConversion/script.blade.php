...
<script>
    ...
    function populateDistrict(state_id) {
        ...
        var value = state_id;
        $('#stateList').update({url: '<?= url('populate-dropdown') ?>?namePrefix=land_conversion&type=district_id&val=' + value + '&label_name=District', data: {'_token': '<?= csrf_token() ?>'}, destination: '.districtList'});
    }
    function populateBlock(district_id) {
        ...
        $('.districtList').update({url: '<?= url('populate-dropdown') ?>?namePrefix=land_conversion&type=block_id&val=' + value + '&label_name=Block/Taluk', data: {'_token': '<?= csrf_token() ?>'}, destination: '.block-list'});
    }
    function populateVillage(block_id) {
        ...
        $('.survey_list_table').css('visibility', 'hidden');
        var value = block_id;
        $('.block-list').update({url: '<?= url('populate-dropdown') ?>?onchange=populateSurvey("",$(this).val())&namePrefix=land_conversion&type=village_id&val=' + value + '&label_name=Village', data: {'_token': '<?= csrf_token() ?>'}, destination: '.village-list'});
    }
    function populateSurvey(reg_id, village_id) {
        ...
        $('.village-list').update({url: '<?= url('transaction/land-conversion/populate-dropdown') ?>?namePrefix=land_conversion&type=survey_id&val=' + value + '&label_name=Survey No.&trxn_type=' + trxn_type, data: {'_token': '<?= csrf_token() ?>'}, destination: '.survey_id'});
    }
...
//        });
        ...
        $('.delete-existing-survey').on('click', function () {
            ...
            if (!confirm("Do you want to delete")) {
                ...
                var delete_url = '<?= url('transaction/land-conversion/delete') ?>' + '/' + survey_id + '/' + conversion_id;
                window.location = delete_url;
            }
        });

        $('#conversion_list_table').DataTable({
            "oLanguage": {
                "sSearch": "",
...
<?php
if ($user_type == 'admin' || (\App\Models\UtilityModel::ifHasPermission('land_conversion_print', $current_user_id))) {
    ?>
                        {
                            "sExtends": "print",
                            "mColumns": [0, 1, 2, 3, 4, 5, 6]
