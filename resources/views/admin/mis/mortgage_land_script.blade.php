<script>
    function populateResistration(trxn_type) {
        ...
        $('.village-list').update({
            url: '<?= url('transaction/hypothecation/populate-dropdown') ?>?namePrefix=hypothecate&type=registration_id&multiple=true&label_name=Registration No.&trxn_type=' +
                trxn_type,
            data: {
                '_token': '<?= csrf_token() ?>'
            },
    ...
    function populateResistrationDetails(value) {
        ...
        //     url: '<?= url('transaction/hypothecation/populate-registrationDetailsTable') ?>?value=' + value,
        //     data: {
        //         '_token': '<?= csrf_token() ?>'
        //     },
        //     destination: '.registration_details'
        // });
        // $('.registration_id').update({
        //     url: '<?= url('transaction/hypothecation/populate-registrationDocumentDetailsTable') ?>?&trxn_type=' +
        //         trxn_type + '&value=' + value,
        //     data: {
        //         '_token': '<?= csrf_token() ?>'
        //     },
        //     destination: '.registration_document_details'
        // });
    }
    ...
    $(function() {
        ...
        $('input[name="hypothecate[value]"]').autoNumeric('init', {
            mDec: 2,
            dGroup: 2
        });
