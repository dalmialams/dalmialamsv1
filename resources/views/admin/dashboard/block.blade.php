                        ...
                        if ($RegDataWithState) {
                            ...
                            foreach ($RegDataWithState as $key => $value) {
                                if ($value['grand_total_tot_cost']) {
                                    ...
                                    $allBlockIds = $allBlockIds.','.$value['id'];
                                    ?>
                                    <tr>
                                        <td  class="text-left">
                                            <?php if ($filter == '') { ?>
                                            <span class="context-menu-one btn btn-neutral"><?php echo $name = App\Models\Common\BlockModel::where(['id' => "{$value['id']}"])->value('block_name'); ?></span>
                                            <?php } elseif ($filter == 'Village') { ?>
                                                <a href="<?= url('dashboard/village?state_id=' . $value['get_state']['id'] .'&district_id=' . $value['get_district']['id'] .'&block_id=' . $value['id'] . '&filter=Village'); ?>" class="context-menu-one btn btn-neutral" tag="<?= $value['id']; ?>">
                                                    <?php echo $name = App\Models\Common\BlockModel::where(['id' => "{$value['id']}"])->value('block_name'); ?>
                                                </a>
                                            <?php } else { 
                                                
                                               
                                                ?>
                                                <a href="<?= url('dashboard/details?state_id=' . $value['get_state']['id'] .'&district_id=' . $value['get_district']['id'] .'&block_id=' . $value['id'] . '&filter=' . $filter); ?>" class="context-menu-one btn btn-neutral" tag="<?= $value['id']; ?>">
                                                    <?php echo $name = App\Models\Common\BlockModel::where(['id' => "{$value['id']}"])->value('block_name'); ?>
                                                </a>
                                            <?php } ?>
                                        </td>
                                        ...
                                        <td style="text-align: right;">
                                            ...
                                            <span class="all_total_cost custom_underline" onmouseover ="totalCostDistribution('<?= $state_id;?>','<?= $district_id;?>','<?= $value['id']; ?>','','<?= $value['id']; ?>')"><?= $value['grand_total_tot_cost'] ?></span></td>
                                    </tr>
                                    <?php
                                }
                            }
                        ...
                        <tr>
                            ...
                            <?php } ?>
                                <th style="text-align: right;">
                                    ...
                                    <span class="all_total_cost custom_underline" onmouseover ="totalCostDistribution('<?= $state_id;?>','<?= $district_id;?>','<?= $allBlockIds; ?>','','total')"><?= $grandTotalCost ?></span></th>
                        </tr>
                    </tfoot>
                </table>
