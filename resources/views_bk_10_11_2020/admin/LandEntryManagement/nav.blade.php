<div class="row " id="noprint">
        ...
        <div class="panel-body">
            <?php if (isset($viewMode) && $viewMode == 'true') { ?>
                ...
                <?php } ?>
				 ...
				 <?php
                if ($user_type == 'admin' || (\App\Models\UtilityModel::ifHasPermission('family_tree_view', $current_user_id) && \App\Models\UtilityModel::ifHasPermission('registration_access', $current_user_id))) {
                    ?>  
                    <a href="<?= isset($reg_uniq_no) && $reg_uniq_no ? url('land-details-entry/family-tree/view?reg_uniq_no=' . $reg_uniq_no . '&view=' . $viewMode) : 'javascript:void(0);' ?>" class="btn <?= isset($section) && ($section == 'family_tree') ? "btn-primary" : "btn-default" ?> mr5">Family Details</a>
                <?php } ?>
    ...
    <!--<div class="pull-right mr5 text-danger" style="font-size: large;"><?= isset($unique_id_number) && $unique_id_number ? $unique_id_number : '' ?></div> -->      
            <?php } else { ?>
				...
				<?php
				if ($user_type == 'admin' || (\App\Models\UtilityModel::ifHasPermission('family_tree_add', $current_user_id) && \App\Models\UtilityModel::ifHasPermission('registration_access', $current_user_id))) {
				?>  
				<a href="<?= isset($reg_uniq_no) && $reg_uniq_no ? url('land-details-entry/family-tree/add?reg_uniq_no=' . $reg_uniq_no) : 'javascript:void(0);' ?>" class="btn <?= isset($section) && ($section == 'family_tree') ? "btn-primary" : "btn-default" ?> mr5">Family Details</a>
				<?php } ?>				
