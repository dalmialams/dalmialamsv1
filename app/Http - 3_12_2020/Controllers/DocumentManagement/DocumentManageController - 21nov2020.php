	...
	public function importShp($table) {
        ...
        if($table){
			...
			if(count($survey_map_data) > 0){
                $msg = '<div class="alert alert-success"><a href="javascript:void(0)" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Sorry, survey no with the registration id already exists</strong></div>';				
				return redirect('document-management/document/shape-file-list')->with('message', $msg);
			}
			}
			}
