...
//        t($posted_data,1);
        ...
        #####################For Grand Total Acer Calculation In Holding##################################
        $getCond = " 1 = 1";
        if(!empty($district_id) && $filter == 'District'){
            $getCond .= " and state_id = '$state_id'";
        }else if(!empty($block_id) && $filter == 'Block/Taluk'){
            $getCond .= " and  district_id = '$district_id'";
        }else if(!empty($village_id) && $filter == 'Village'){
            $getCond .= " and block_id = '$block_id'";
        }
        ...
        if ($context == 'holding') {
            ...
            if ($this->user_type == 'admin') {
                ...
                $grand_total_tot_cost = RegistrationModel::selectRaw('sum(tot_cost) as grand_total_tot_cost')->whereRaw($total_area_cond)->get()->toArray();
                $getGrandTotalAcer = RegistrationModel::getTotalAreaCalculation($getCond);
            } else {
                if (!empty($state_id) && empty($district_id) && empty($block_id) && empty($village_id)) {
                    $total_area_cond .= " and  district_id in($assignStateDistrictLists)";
                    $getCond .= " and  district_id in($assignStateDistrictLists)";
                }
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
                $grand_total_tot_cost = RegistrationModel::selectRaw('sum(tot_cost) as grand_total_tot_cost')->whereRaw($total_area_cond)->get()->toArray();
                $getGrandTotalAcer = RegistrationModel::getTotalAreaCalculation($getCond);
            ...
            $total_area_details['grand_total_tot_cost'] = $grand_total_tot_cost[0]['grand_total_tot_cost'];
            //t($getGrandTotalAcer);
        ...
        $this->data['getGrandTotalAcer'] = isset($getGrandTotalAcer['grand_total_tot_area_acer']) ? $getGrandTotalAcer['grand_total_tot_area_acer'] : '';
        $this->data['context'] = $context;
        $this->data['dropdown'] = $dropdown;
        $this->data['grand_total_cost_all'] = isset($grand_total_cost_all) ? $grand_total_cost_all : '';
        return view('admin.dashboard.context.holding', $this->data);
    }
