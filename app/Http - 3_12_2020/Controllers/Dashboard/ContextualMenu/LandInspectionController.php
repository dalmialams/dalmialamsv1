...
class LandInspectionController extends Controller {
    ...
    //    t($assignStateDistrictLists, 1);
        if ($context == 'inspection') {
            if ($this->user_type == 'admin') {
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
                $inspection_details = \App\Models\Transaction\InspectionModel::selectRaw("array_to_string(array_agg((survey_id)), ',') as survey_id_str")->where($finalcond)->get()->toArray();
            } else {
                if (!empty($state_id) && empty($district_id) && empty($block_id)) {
                    $total_area_cond .= " and  district_id in($assignStateDistrictLists)";
                    $inspection_details = \App\Models\Transaction\InspectionModel::selectRaw("array_to_string(array_agg((survey_id)), ',') as survey_id_str")->whereRaw("district_id in ($assignStateDistrictLists)")->get()->toArray();
                } else {
                    $inspection_details = \App\Models\Transaction\InspectionModel::selectRaw("array_to_string(array_agg((survey_id)), ',') as survey_id_str")->where($finalcond)->get()->toArray();
                }
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
            ...
            if ($inspection_details) {
                $survey_id_str = isset($inspection_details[0]['survey_id_str']) ? $inspection_details[0]['survey_id_str'] : '';
            }
        ...
        return view('admin.dashboard.context.inspection', $this->data);
    }
    ...
    public function surveyTotalAreaDetails($survey_id_str) {
        ...
        $total_under_operation_area_acre = 0;
        $total_under_operation_area_hectare = 0;

        $total_to_be_inspected_area_acre = 0;
        $total_to_be_inspected_area_hectare = 0;

        $total_inspected_area_acre = 0;
        $total_inspected_area_hectare = 0;

        $total_encroached_area_acre = 0;
        $total_encroached_area_hectare = 0;

        $total_encroached_free_area_acre = 0;
        $total_encroached_free_area_hectare = 0;
        ...
        foreach ($survey_id_arr as $key => $value) {
            ...
            $inspection_status = isset($survey_info['inspection_status']) ? $survey_info['inspection_status'] : '';
            $under_operation_status = isset($survey_info['operation_status']) ? $survey_info['operation_status'] : '';
            $encroached = \App\Models\Transaction\InspectionModel::where(['survey_id' => $value])->value('encroachment');
            ...
            if ($area_unit && $purchased_area) {
                ...
                if ($inspection_status == 'Y') { //applied
                    if ($area_unit != 'CD00094') {
                        $total_inspected_area_acre += $purchased_area * $purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $total_inspected_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $total_inspected_area_hectare += $purchased_area * $purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $total_inspected_area_hectare += $purchased_area;
                    }
                } else {
                    if ($area_unit != 'CD00094') {
                        $total_to_be_inspected_area_acre += $purchased_area * $purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $total_to_be_inspected_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $total_to_be_inspected_area_hectare += $purchased_area * $purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $total_to_be_inspected_area_hectare += $purchased_area;
                    }
                }

                if ($under_operation_status == 'Y') { //under opertation
                    if ($area_unit != 'CD00094') {
                        $total_under_operation_area_acre += $purchased_area * $purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $total_under_operation_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $total_under_operation_area_hectare += $purchased_area * $purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $total_under_operation_area_hectare += $purchased_area;
                    }
                }

                if ($encroached == 'Y') { //under opertation
                    //exit;
                    if ($area_unit != 'CD00094') {
                        $total_encroached_area_acre += $purchased_area * $purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $total_encroached_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $total_encroached_area_hectare += $purchased_area * $purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $total_encroached_area_hectare += $purchased_area;
                    }
                } else {
                    if ($area_unit != 'CD00094') {
                        $total_encroached_free_area_acre += $purchased_area * $purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $total_encroached_free_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $total_encroached_free_area_hectare += $purchased_area * $purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $total_encroached_free_area_hectare += $purchased_area;
                    }
                }
            }
        }

        $returnArray['total_under_operation_area_acre'] = $total_under_operation_area_acre;
        $returnArray['total_under_operation_area_hectare'] = $total_under_operation_area_hectare;

        $returnArray['total_to_be_inspected_area_acre'] = $total_to_be_inspected_area_acre;
        $returnArray['total_to_be_inspected_area_hectare'] = $total_to_be_inspected_area_hectare;

        $returnArray['total_inspected_area_acre'] = $total_inspected_area_acre;
        $returnArray['total_inspected_area_hectare'] = $total_inspected_area_hectare;

        $returnArray['total_encroached_area_acre'] = $total_encroached_area_acre;
        $returnArray['total_encroached_area_hectare'] = $total_encroached_area_hectare;

        $returnArray['total_encroached_free_area_acre'] = $total_encroached_free_area_acre;
        $returnArray['total_encroached_free_area_hectare'] = $total_encroached_free_area_hectare;
