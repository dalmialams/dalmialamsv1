        ...
        if ($states) {
            foreach ($states as $key => $val) {
                ...
                if ($this->user_type !== 'admin') {
                    ...
                    $finalcond = $cond . " AND state_id = '{$val['id']}'".$where;
                }
...
//        t($RegDataWithState);die("Uuuu");
	...
	{	
		...
		$sql .= " where 1=1 AND (state_name ='Chandigarh' OR state_name ='Himachal Pradesh' OR state_name ='Manipur' OR state_name ='Meghalaya' OR state_name ='Nagaland' OR state_name ='Sikkim' OR state_name ='Dadra & Nagar Haveli' OR state_name ='Delhi') AND (wkb_geometry is not null)";
		$sql = $sql . ") As lp
