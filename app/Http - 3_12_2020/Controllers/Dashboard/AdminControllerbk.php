...
//t($this->assigned_states,1);
        ...
        if ($states) {
            foreach ($states as $key => $val) {
                ...
                if ($this->user_type !== 'admin') {
                    $finalcond = $cond . " AND state_id = '{$val['id']}' and district_id in($assigned_districts)";
                } else {
                    $finalcond = $cond . " AND state_id = '{$val['id']}'";
                }
				$payment_cond = $finalcond . "AND pay_type!='' And \"T_REGISTRATION\".fl_archive ='N'";
                ...
                $grand_total_tot_cost = RegistrationModel::selectRaw('sum(amount) as grand_total_tot_cost')->join('PAYMENT','PAYMENT.registration_id','=',"REGISTRATION.id",'left')->whereRaw($payment_cond)->get()->toArray();
