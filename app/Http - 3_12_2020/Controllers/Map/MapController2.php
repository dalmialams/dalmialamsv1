...
//echo "hiii";die;
		...
		//$plot_survey_data = DB::table('SURVEY')->orderBy('id','asc')->get()->toArray();
		
		//t($plot_survey_data);die;
	...
	{	
		...
		$whereData = array(array('state_id',$state_id) , array('fl_archive','N')); 
			...
			$zone = isset($classification_result[0]['cd_desc'])? $classification_result[0]['cd_desc'] : '';
                ...
                if (!empty($plotDetails)) {
                   ...
                   // $PLS_PLOT_TYPE = $this->all_function->get_dynamic_dropdown('plot_type', $PLS_PLOT_TYPE, true);
                    ...
                    <tr><td><b>State:</b></td><td>' . $plotDetails[0]['state'] . '</td></tr>
                    <tr><td><b>District:</b></td><td>' . $plotDetails[0]['district'] . '</td></tr>
                    <tr><td><b>Village:</b></td><td>' . $plotDetails[0]['village'] . '</td></tr>
					...
					<tr><td></td><td><a class="btn btn-success btn-xs" target="_blank" href="'.URL("land-details-entry/registration/view?reg_uniq_no=".$registration_id."&view=true").'">View</a></td></tr>
		...
		$geomjson = $request->input('coordinate');
					...
					//$ogc_fid = $value['ogc_fid'];
							...
							if (!empty($plt) && $plt != 'null') {
								$sql = $sql . " AND \"ogc_fid\" IN='$ogc_fid'";
							}
	...
	{	
		...
		$whereData = array(array('district_id',$district_id) , array('fl_archive','N')); 
...
//$survey_id = '102/1';		
	...
	{	
		...
		$whereData = array(array('block_id',$block_id) , array('fl_archive','N')); 
...
//echo $filename;die;			
        ...
        if (1) {
            ...
            if ($vill) {
                ...
                $sql3 = "SELECT st_extent(wkb_geometry) FROM \"T_SURVEY_MAP\"";
            }
