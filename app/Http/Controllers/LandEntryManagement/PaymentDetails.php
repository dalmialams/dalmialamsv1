...
class PaymentDetails extends Controller {
    ...
    public function add(Request $request) {
        ...
        $res = PaymentModel::where(['registration_id' => $reg_uniq_no, 'fl_archive' => "N"])->get()->toArray();
    ...
    public function edit(Request $request) {
        ...
        if ($payment_no) {
            if ($this->user_type !== 'admin') {
                if ($this->hasPermission('registration_access') && !$this->hasPermission('payment_details_edit')) {
    ...
    public function processData(Request $request) {
        ...
        $reg_data_update = array(
            'tot_cost' => $posted_data['payment']['amount']
        );
        ...
        if (!empty($doc_file)) {
            for($i = 0; $i < count($doc_file); $i++){
                $temp_doc = $doc_file[$i];
                $file_name = $temp_doc->getClientOriginalName();
                $file_name = stristr($file_name, '.', true);
                $tempExt = $temp_doc->getClientOriginalExtension();
                $new_file_name = $file_name . '_' . time() . '.' . $tempExt;
                $file_path[] = str_replace('\\', '/', $this->upload_dir . '/' . $new_file_name);
                $ext[] = $tempExt;
                if (in_array($tempExt, $this->allowed_types)) {
                    $upload = UtilityModel::uploadFile($temp_doc, $this->upload_dir, $new_file_name);
                } else {
                    ...
                    if ($posted_data['payment_no']) {
                        return redirect('land-details-entry/payment/edit?reg_uniq_no=' . $registration_id . '&payment_no=' . $posted_data['payment_no'])->with('message', $msg)->withInput();
                    } else {
        ...
        if ($posted_data['payment_no']) {
            ...
            if ($validator->fails()) {
                return redirect('land-details-entry/payment/edit?reg_uniq_no=' . $posted_payment_data['registration_id'] . '&payment_no=' . $posted_data['payment_no'])
            ...
            RegistrationModel::where(['id' => $posted_payment_data['registration_id']])->update($reg_data_update);
            if ($update_id) {
                    ...
                    if ($this->user_type !== 'admin') {
                        if ($this->hasPermission('registration_access') && $this->hasPermission('lease_details_add')) {
                            if (isset($this->data['purchase_type_id']) && $this->data['purchase_type_id'] == 'CD00144') {
                                ...
                                return redirect('land-details-entry/payment/edit?reg_uniq_no=' . $registration_id . '&payment_no=' . $posted_data['payment_no']);
                            }
                        } else if ($this->hasPermission('registration_access') && $this->hasPermission('geo_tag_add')) {
                            return redirect('land-details-entry/survey-lat-long/add?reg_uniq_no=' . $registration_id)->with('message', $msg);
                        } else if ($this->hasPermission('registration_access') && $this->hasPermission('transaction_details_access')) {
            ...
            $data_to_insert = $posted_payment_data;
            $data_to_insert['path'] = isset($file_path) ? json_encode($file_path, JSON_UNESCAPED_SLASHES) : "";
            $data_to_insert['file_type'] = isset($ext) ? json_encode($ext) : "";
                    ...
                    if ($this->user_type !== 'admin') {
                        if ($this->hasPermission('registration_access') && $this->hasPermission('lease_details_add')) {
                            if (isset($this->data['purchase_type_id']) && $this->data['purchase_type_id'] == 'CD00144') {
                                ...
                                return redirect('land-details-entry/payment/add?reg_uniq_no=' . $registration_id);
                            }
                        } else if ($this->hasPermission('registration_access') && $this->hasPermission('geo_tag_add')) {
                            return redirect('land-details-entry/survey-lat-long/add?reg_uniq_no=' . $registration_id)->with('message', $msg);
                        } else if ($this->hasPermission('registration_access') && $this->hasPermission('transaction_details_access')) {
...
//                    }
    ...
    public function paymentDelete(Request $request) {
        ...
        if (!empty($reg_uniq_no) && !empty($payment_no)) {
            ...
            if ($delete_id) {
                ...
                return redirect('land-details-entry/payment/add?reg_uniq_no=' . $reg_uniq_no)->with('message', $msg);
            }
        } else {
