...
//        $this->data['data']['jsArr'][] = 'assets/js/autoNumeric-min.js';
        ...
        if ($posted_data) {
            ...
            //echo $cond;
            //exit;
            $registrations = RegistrationModel::whereRaw($cond)->orderBy('id', 'ASC')->get()->toArray(); //t($registrations);exit;
	  ...
	  if(!empty($reg_id_str))
            {
		//echo $reg_id_str;exit;
		$reg_id_str_array = explode("','",trim($reg_id_str,"'"));
		// t($reg_id_str_array,1);
               ...
               //$survey_number_details = DB::select(DB::raw("select * from T_SURVEY where  fl_archive = 'N' and registration_id in ($reg_id_str)"));
		 ...
		 //t($survey_number_details, 1);
            ...
            if ($registrations) {
                foreach ($registrations as $key => $value) {
                    ...
                    $reg_array[] = [
                        ...
                        $regn_date, $sub_registrar_office, $value['vendor'], $purchased_area_unit, number_format((double)$value['tot_area'], 4, '.', ''), (double) $value['tot_cost'],
            ...
            if ($survey_number_details) {
                foreach ($survey_number_details as $key => $value) {
                    ...
                    $survey_array[] = [
                        $value['registration_id'], $survey_no, $state, $district, $block, $village, $doc_reg_no, $extend_unit, number_format((double)$value['total_area'], 4, '.', ''), number_format((double)$value['purchased_area'], 4, '.', ''), $purpose, $classification, $sub_classification,
...
//t($document_reg_array,1);
            ...
            if ($payment_details) {
                foreach ($payment_details as $key => $value) {
                    ...
                    $payment_array [] = [
                        $value['registration_id'], $payment_type, $payment_mode, (double) $value['amount'], $value['reference_no'], $value['pay_date'], $value['pay_bank'], $value['description'], $value['path']
                    ];
                }
            }else{
				$payment_array [] = [
...
//        $this->data['data']['jsArr'][] = 'assets/js/autoNumeric-min.js';
        ...
        if ($posted_data) {
            ...
            $i=0;
            $allSearchData = $posted_data['registration'];
            $allSearchData1 = $posted_data['coversion_parent'];
                ...
                foreach ($dispute_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
							...
							if($survey_id !='')
							{
								$reg_arr = SurveyModel::join('REGISTRATION','REGISTRATION.id','=','SURVEY.registration_id','left')->get()->toArray();
                ...
                foreach ($parent_details as $key => $value) {
                    ...
                    if ($registration_res) {
                        foreach ($registration_res as $r_key => $r_value) {
							...
							//eng reg details
                            $parent_array[] = [
                                $value['id'], $value['converted_registration_id'], $transaction_date, $value['old_registration_id'],$reg_id, $purchaser_name, $legal_entity, $sub_registrar, (double) $purchased_area, (double) $cost,
                                $state, $district, $block, $village
                            ];
                        }
                    }
