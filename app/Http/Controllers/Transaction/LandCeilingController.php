    ...
    public function ceilingSurvey($village_id = null, $type = null, $trxn_type = 'N', $from_mis = null) {
        ...
        if (!$from_mis) {
            if ($village_id && $type == 'ajax') {
                ...
                $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
                if ($registration_ids) {
                    $survey_list = SurveyModel::where(['land_ceiling_status' => $trxn_type])->whereRaw("registration_id in ($registration_ids)")->where(['fl_archive' => 'N'])->orderBy('id')->pluck('survey_no', 'id')->toArray();
                } else {
                    $survey_list = '';
                }
