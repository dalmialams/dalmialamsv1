    ...
    public function __construct() {
        ...
        if ($this->user_type !== 'admin') {
            ...
            if ($this->assigned_states) {
                ...
                //$reg_id_arr = RegistrationModel::whereRaw("district_id in ($assgined_districts_str)")->orderBy('id')->pluck('id', 'id')->toArray();
            }
        }
...
//                    }
    ...
    public function landEchangeSurvey($village_id = null, $type = null) {
        ...
        if ($village_id && $type == 'ajax') {
            ...
            $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
            if ($registration_ids) {
                $survey_list = SurveyModel::where(['land_exchange_status' => 'N'])->whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'id')->toArray();
            } else {
                $survey_list = '';
            }
    ...
    public function deleteExchangeSurvey($survey_id, $land_exchange_id) {
        ...
        return redirect('transaction/land-exchange/edit/' . $land_exchange_id)->with('message', $msg);
    }
