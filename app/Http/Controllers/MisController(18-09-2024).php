        ...
        if ($posted_data) {
            ...
            if ($registrations) {
                foreach ($registrations as $key => $value) {
                    ...
                    $reg_array[] = [
                        ...
                        $regn_date, $sub_registrar_office, $value['vendor'], $purchased_area_unit, number_format((float)$value['tot_area'], 4, '.', ''), number_format((float)($purchased_area_unit_conv * $value['tot_area']), 4, '.', ''), (float) $value['tot_cost'],
                        $state, $district, $block, $village, $source
                    ];
                }
            }
            ...
            if ($survey_number_details) {
                foreach ($survey_number_details as $key => $value) {
                    ...
                    $survey_array[] = [
                        $value['registration_id'], $survey_no, $state, $district, $block, $village, $doc_reg_no, $extend_unit, number_format((float)$value['total_area'], 4, '.', ''), number_format((float)$value['purchased_area'], 4, '.', ''), $purpose, $classification, $sub_classification,
                        ...
                        $land_exchange_status, $survey_source
                    ];
            ...
            if ($document_details) {
                foreach ($document_details as $key => $value) {
                    ...
                    $document_array[] = [
                        $value['registration_id'], $doc_type, $value['physical_location'], $value['path'], $value['remarks']
                    ];
            ...
            if ($payment_details) {
                foreach ($payment_details as $key => $value) {
                    ...
                    $payment_array[] = [
                        $value['registration_id'], $payment_type, $payment_mode, (float) $value['amount'], $value['reference_no'], $value['pay_date'], $value['pay_bank'], $value['description'], $value['path']
                    ];
                }
            } else {
                $payment_array[] = [
        ...
        if ($posted_data) {
                ...
                foreach ($dispute_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            $dispute_array[] = [
                                $s_value['disputes_id'], $reg_id, $s_value['survey_no'], $dispute_date, $reminder_date, $hearing_date, $litigation_type, $value['description'],
                ...
                foreach ($land_ceiling_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            $ceiling_array[] = [
                                $s_value['ceiling_id'], $reg_id, $s_value['survey_no'], $transaction_type, $transaction_date, $s_value['remarks'],
                                $state, $district, $block, $village
                            ];
                        }
                    }
                ...
                foreach ($land_conversion_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            $conversion_array[] = [
                                $s_value['conversion_id'], $reg_id, $s_value['survey_no'], $transaction_type, $transaction_date, $s_value['remarks'],
                                $state, $district, $block, $village
                            ];
                        }
                    }
                ...
                foreach ($land_exchnage_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            $exchange_array[] = [
                                $s_value['land_exchange_id'], $reg_id, $s_value['survey_no'], $transaction_date, $s_value['transferee'], $date_of_exchange, $s_value['remarks'],
                                $state, $district, $block, $village
                            ];
                        }
                    }
            ...
            if ($land_inspection_details) {
                foreach ($land_inspection_details as $key => $value) {
                    ...
                    $village = \App\Models\Common\VillageModel::where(['id' => $value['village_id']])->value('village_name');
                    $inspection_array[] = [
                        $value['id'], $reg_id, $survey_no, $transaction_date, $inspection_date, $encroachment, $encroachment_type, $value['description'],
                        $state, $district, $block, $village
                    ];
                }
            }
                ...
                foreach ($land_reservation_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            //eng reg details
                            $reservation_array[] = [
                                $s_value['reservation_id'], $reg_id, $s_value['survey_no'], $transaction_type, $transaction_date, $s_value['remarks'],
                                $state, $district, $block, $village
                            ];
                        }
                    }
            ...
            if ($under_opertaion_details) {
                foreach ($under_opertaion_details as $key => $value) {
                    ...
                    $village = \App\Models\Common\VillageModel::where(['id' => $value['village_id']])->value('village_name');
                    $operation_array[] = [
                        $value['id'], $reg_id, $survey_no, $transaction_date, $operation_type, $value['description'],
                        $state, $district, $block, $village
                    ];
                }
            }
            ...
            if ($lease_details) {
                foreach ($lease_details as $key => $value) {
                    ...
                    $lease_array[] = [
                        $value['id'], $value['registration_id'], $value['lease_name'], $value['lessor_name'], (float) $value['lease_monthly_amount'],
            ...
            if ($payment_details) {
                foreach ($payment_details as $key => $value) {
                    ...
                    $payment_type = \App\Models\Common\CodeModel::where(['id' => $value['pay_type'], 'cd_type' => 'payment_type'])->value('cd_desc');
                    $payment_array[] = [
                        $value['id'], $value['registration_id'], $transaction_date, $payment_type, $pay_mode, (float) $value['amount'], $value['reference_no'],
                ...
                foreach ($mining_lease_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            //eng reg details
                            $mining_array[] = [
                                $s_value['mining_lease_id'], $reg_id, $s_value['survey_no'], $transaction_type, $transaction_date, $s_value['go'], $s_value['remarks'],
                                $state, $district, $block, $village
                            ];
                        }
                    }
                ...
                foreach ($mutation_details as $key => $value) {
                    ...
                    if ($survey_res) {
                        foreach ($survey_res as $s_key => $s_value) {
                            ...
                            //eng reg details
                            $mutation_array[] = [
                                $value['id'], $patta_no, $patta_owner, $value['new_patta_no'], $value['new_patta_owner'], $reg_id, $s_value['survey_no'], $transaction_date,
                                $state, $district, $block, $village
                            ];
                        }
                    }
                ...
                foreach ($hypothetication_details as $key => $value) {
                    ...
                    if ($registration_res) {
                        foreach ($registration_res as $r_key => $r_value) {
                            ...
                            $hypothetication_array[] = [
                                $value['id'], $transaction_date, $hypo_name, $trxn_type, $r_value['registration_id'], $hypo_with, $date, (float) $value['value'], $value['description']
                            ];
                        }
                    }
                }
                ...
                foreach ($parent_details as $key => $value) {
                    ...
                    if ($registration_res) {
                        foreach ($registration_res as $r_key => $r_value) {
                            ...
                            //eng reg details
                            $parent_array[] = [
                                $value['id'], $value['converted_registration_id'], $transaction_date, $value['old_registration_id'], $reg_id, $purchaser_name, $legal_entity, $sub_registrar, (float) $purchased_area, (float) $cost,
                                $state, $district, $block, $village
                            ];
                        }
                    }
