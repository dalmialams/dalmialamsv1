...
class PattaController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $this->data['pageHeading'] = 'Patta Record <span class="text-danger" style="color: #db5565;" >Add </span>';
    ...
    public function pattaList(Request $request)
    {
        ...
        $patta_no_info = PattaModel::pluck('patta_no', 'patta_no')->toArray();
    ...
    public function getAllPattaSurvey($registration_id = null, $village_id = null, $type = null)
    {
        ...
        if ($registration_id) {
            ...
            if ($type) {
                ...
                $survey_list = SurveyModel::whereRaw("id in ($surveyIds)")->orderBy('id')->pluck('survey_no', 'id')->toArray();
            } else {
                $survey_list = '';
            }
            ...
            $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
            if ($registration_ids) {
                $survey_list = SurveyModel::where(['mutation_status' => 'N', 'patta_entry_status' => 'N'])->whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'id')->toArray();
            } else {
                $survey_list = '';
            }
            $survey_list = array_merge(array('' => 'select'), $survey_list);
            ...
            if ($this->user_type == 'admin') {
                ...
                if ($this->assigned_states) {
                    ...
                    if ($assgined_districts_str) {
                        ...
                        $registration_ids = isset($registration_ids[0]['id_str']) ? $registration_ids[0]['id_str'] : '';
                        $survey_list = SurveyModel::whereRaw("registration_id in ($registration_ids)")->orderBy('id')->pluck('survey_no', 'survey_no')->toArray();
                    }
                }
            }
        }
