...
class CostDistributionController extends Controller {
    ...
    public function index(Request $request) {
		...
		$purchase_type = $posted_data['purchase_type'] ? $posted_data['purchase_type']: '';
		
		$where="";
		if($purchase_type =='leased')
		{
			$where .=" and purchase_type_id='CD00144'";
		}
		else if($purchase_type =='purchased')
		{
			$where .=" and purchase_type_id !='CD00144'";
		}
		else
        ...
        $total_area_cond = " 1 = 1 AND fl_archive = 'N'";
		...
		$this->data['state_name'] =$this->getStatesnameById($state_id);
    ...
    public function getAllPaymentCostDistribution($reg_id_str = null) {
        ...
        if ($reg_id_str) {
            ...
            foreach ($allPaymentType as $key => $val) {
                if ($key == 'CD00027') {
                    $allPaymentDetails = PaymentModel::selectRaw('sum(amount) as grand_total_tot_cost')->whereRaw("registration_id in ($reg_id_str)")->where(['pay_type' => $key, 'fl_archive' => 'N'])->get()->toArray();
