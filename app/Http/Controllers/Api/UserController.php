...
class UserController  {
    ...
    public function createUser(Request $request)
    {
        ...
        if(!empty($postData['name']) && !empty($postData['email'])){
            ...
            if(!empty($role)){
                ...
                $existMasterUser = MasterUserModel::where('email', $postData['email'])->get()->toArray();
                if(!empty($existUser) || !empty($existMasterUser)){
                    ...
                    $insertArr = array(
                        ...
                        "crt_id" => $postData['created_id'],
                        "upd_id" => $postData['created_id']
                    );
    ...
    public function updateStatus(Request $request)
    {
        ...
        if(!empty($postData['email']) && isset($postData['active'])){
            UserModel::where('user_name',$postData['email'])->update(['active' => $postData['active'] , 'fl_archive' => ($postData['active'] == true || $postData['active'] == 'true') ? 'N' : 'Y']);
