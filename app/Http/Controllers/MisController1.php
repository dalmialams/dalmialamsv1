 ...
 * @author user-98-pc
 */
class MisController1 extends Controller {
...
//        $this->data['data']['jsArr'][] = 'assets/js/autoNumeric-min.js';
        ...
        if ($posted_data) {
            ...
            $posted_disputes_data = isset($posted_data['land_ceiling']) ? $posted_data['land_ceiling'] : '';
            $ceiling_cond = ' 1=1';

            $dispute_details = \App\Models\Transaction\DisputesModel::whereRaw($cond)->orderBy('id', 'ASC')->get()->toArray();
            //t($dispute_details);
            $dispute_array = [
                [
            ...
            if ($posted_ceiling_data) {
                ...
                $trxn_from_date = isset($posted_ceiling_data['date_from']) && $posted_ceiling_data['date_from'] ? $posted_ceiling_data['date_from'] : '';
                $trxn_from_date = ($trxn_from_date) ? new \DateTime($trxn_from_date) : '';
                $trxn_from_date = ($trxn_from_date) ? $trxn_from_date->format('Y-m-d') : '';
                $trxn_to_date = isset($posted_ceiling_data['date_to']) && $posted_ceiling_data['date_to'] ? $posted_ceiling_data['date_to'] : '';
            ...
            $land_conversion_details = \App\Models\Transaction\LandConversionModel::whereRaw($cond)->orderBy('id', 'ASC')->get()->toArray();
            $conversion_array = [
                [
                    'Transaction ID', 'Survey No.', 'Transaction Type', 'Transaction Date', 'Remarks',
                    'State', 'District', 'Block/Taluk', 'Village'
            ...
            $land_exchnage_details = \App\Models\Transaction\LandExchangeModel::whereRaw($cond)->orderBy('id', 'ASC')->get()->toArray();
            ...
            if ($land_exchnage_details) {
                foreach ($land_exchnage_details as $key => $value) {
                    $survey_res = \App\Models\Transaction\LandExchangeSurveyModel::where(['land_exchange_id' => $value['id']])->get()->toArray();
                    $transaction_date = ($value['created_at']) ? date('d/m/Y', strtotime($value['created_at'])) : '';
                    $state = \App\Models\Common\StateModel::where(['id' => $value['state_id']])->value('state_name');
                    $district = \App\Models\Common\DistrictModel::where(['id' => $value['district_id']])->value('district_name');
                    $block = \App\Models\Common\BlockModel::where(['id' => $value['block_id']])->value('block_name');
            ...
            $land_inspection_details = \App\Models\Transaction\InspectionModel::whereRaw($cond)->orderBy('id', 'ASC')->get()->toArray();
            ...
            $land_reservation_details = \App\Models\Transaction\LandReservationModel::whereRaw($cond)->orderBy('id', 'ASC')->get()->toArray();
            ...
            $lease_details = \App\Models\LeaseManagement\LeaseModel::whereRaw("registration_id in ($reg_id_str)")->whereRaw($lease_cond)->where(['fl_archive' => 'N'])->get()->toArray();
            // t($lease_details, 1);
            $lease_array = [
                [
            ...
            $payment_details = PaymentModel::whereRaw("registration_id in($reg_id_str)")->whereRaw($payment_cond)->where(['fl_archive' => 'N'])->orderBy('registration_id', 'ASC')->get()->toArray();
            $payment_array = [
                [
            ...
            $parent_details = \App\Models\Transaction\ParentConversionModel::where(['fl_archive' => 'N'])->whereRaw("converted_registration_id in($reg_id_str)")->orderBy('id', 'ASC')->get()->toArray();
