...
class DocumentManageController extends Controller {
	...
	public function shape_file_list(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='survey_plot' and fl_archive = 'N' order by created_at desc") );
	...
	public function mining_lease_boundary(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='mining_lease_boundary' and fl_archive = 'N' order by created_at desc") );
	...
	public function colony_boundary(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='colony_boundary' and fl_archive = 'N' order by created_at desc") );
	...
	public function plant_boundary(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='plant_boundary' and fl_archive = 'N' order by created_at desc") );
	...
	public function truckyard(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='truckyard' and fl_archive = 'N' order by created_at desc") );
	...
	public function railway_sliding_boundary(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='railway_sliding_boundary' and fl_archive = 'N' order by created_at desc") );
	...
	public function approach_road(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='approach_road' and fl_archive = 'N' order by created_at desc") );
	...
	public function conveyor_belt(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='conveyor_belt' and fl_archive = 'N' order by created_at desc") );
	...
	public function railway_track(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='railway_track' and fl_archive = 'N' order by created_at desc") );
	...
	public function crusher_location(){
		...
		$this->data['shpHisTables'] = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where shp_type='crusher_location' and fl_archive = 'N' order by created_at desc") );
	...
	public function importShp($table,$shp_type,$data_id) {
        ...
        if($table){
			...
			$chk_village_data = DB::select( DB::raw("select * from \"T_VILLAGE\" where village_name = '".$val->village."'") );	
            $chk_village_id = '';
			...
			$survey_map_data = DB::select( DB::raw("select * from \"T_SURVEY_MAP\" where village_id = '".$chk_village_id."' and survey_no = '".$val->surveyno."'") );
			//dd(DB::getQueryLog());
			if(count($survey_map_data) > 0){
				//echo $val->reg_id .'   ----  '.$val->surveyno;die;
								...
								foreach($exist_temp_table as $temp_table){
									...
									if($temp_table->table_name == 'temp_mining_lease_boundary' && $shp_type =='mining_lease_boundary'){
										...
										if($temp_table->table_name == 'temp_railway_sliding_boundary'){
									      $sql = DB::select( DB::raw("DROP TABLE shptemp.temp_railway_sliding_boundary") );
										}
										break;
								   ...
								   // if (!ini_get('safe_mode'))
									...
									$command = '"D:\\PostgreSQL\\9.5\\bin\\shp2pgsql" -d "' . base_path().'\\' . $dir . $shpfile . '" shptemp.temp_railway_sliding_boundary | "D:\\PostgreSQL\\9.5\\bin\\psql" -h localhost -d "DALMIA_LAMS" -U postgres';
									}elseif($shp_type =='approach_road'){
	...
	public function update_shape_file(Request $request)
	{
		...
		$shp_history_data = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where id='$shpId'") );
		...
		$shape_history_data = DB::select( DB::raw("select * from \"T_SHP_HISTORY\" where id = '".$data_id."' and shp_type='".$type."'") );
		 ...
		 $file = File::files(public_path('assets/files/data_export_shp'));
			...
			if ($filename) {	
			   ...
			   $str = '"D:\\PostgreSQL\\9.5\\bin\\pgsql2shp" -f "D:\wamp\www\dalmia-lams\public\assets\files\data_export_shp\\' . $filename . '" -h localhost -u postgres -P admin@123 DALMIA_LAMS shptemp.'.$table.' 2>&1'; 
	...
	public function manage_shape_data($shp_type) {
       ...
       $shape_data = array();
	   if($shp_type == 'mining_lease_boundary'){
		    $shape_data = DB::table('LEASE_BOUNDARY')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'plant_boundary'){
		    $shape_data = DB::table('PLANT_BOUNDARY')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'colony_boundary'){
		    $shape_data = DB::table('COLONY_BOUNDARY')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'truckyard'){
		    $shape_data = DB::table('TRUCKYARD')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'railway_sliding_boundary'){
		    $shape_data = DB::table('RAILWAY_SLIDING')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'approach_road'){
		    $shape_data = DB::table('APPROACH_ROAD')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'conveyor_belt'){
		    $shape_data = DB::table('CONVEYOR_BELT')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'railway_track'){
		    $shape_data = DB::table('RAILWAY_LINE')->orderBy('ogc_fid','desc')->get();
	   }elseif($shp_type == 'crusher_location'){
		    $shape_data = DB::table('CRUSHER_LOCATION')->orderBy('ogc_fid','desc')->get();
	   }
