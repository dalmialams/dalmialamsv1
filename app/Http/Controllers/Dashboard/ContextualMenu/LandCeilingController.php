...
//        t($posted_data,1);
        ...
        if ($context == 'ceiling') {
            ...
            if ($this->user_type == 'admin') {
                ...
                $allCeilingData = \App\Models\Transaction\LandCeilingModel::selectRaw("array_to_string(array_agg(QUOTE_LITERAL(id)), ',') as id_str")->where($finalcond)->get()->toArray();
            } else {
                if (!empty($state_id) && empty($district_id) && empty($block_id) && empty($village_id)) {
                    $total_area_cond .= " and  district_id in($assignStateDistrictLists)";
                    $allCeilingData = \App\Models\Transaction\DisputesModel::selectRaw("array_to_string(array_agg(QUOTE_LITERAL(id)), ',') as id_str")->whereRaw("district_id in ($assignStateDistrictLists)")->get()->toArray();
                } else {
                    $allCeilingData = \App\Models\Transaction\DisputesModel::selectRaw("array_to_string(array_agg(QUOTE_LITERAL(id)), ',') as id_str")->where($finalcond)->get()->toArray();
                }
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
            }

            $allCeilingIds = isset($allCeilingData[0]['id_str']) ? $allCeilingData[0]['id_str'] : '';

            if ($allCeilingIds) {
                $ceilingSurveyData = \App\Models\Transaction\LandCeilingSurveyModel:: whereRaw("ceiling_id in ($allCeilingIds)")->get()->toArray();

                $survey_id = [];
                foreach ($ceilingSurveyData as $key => $value) {
        ...
        return view('admin.dashboard.context.ceiling', $this->data);
    }
    ...
    public function surveyTotalAreaDetails($survey_id_str) {
        ...
        $obtain_total_ceiling_area_acre = 0;
        $obtain_total_ceiling_area_hectare = 0;
        $applied_total_ceiling_area_acre = 0;
        $applied_total_ceiling_area_hectare = 0;
        ...
        foreach ($survey_id_arr as $key => $value) {
            ...
            if ($survey_info['land_ceiling_status'] == 'O') {
                if ($area_unit && $purchased_area) {
                    $obtain_purchased_area_unit_value = \App\Models\Common\ConversionModel::select('convers_value_acer', 'convers_value_heactor')->where(['code_id' => "$area_unit", 'fl_archive' => 'N'])->get()->toArray();
                    // t($purchased_area_unit_value);
                    if ($area_unit != 'CD00094') {
                        $obtain_total_ceiling_area_acre += $purchased_area * $obtain_purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $obtain_total_ceiling_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $obtain_total_ceiling_area_hectare += $purchased_area * $obtain_purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $obtain_total_ceiling_area_hectare += $purchased_area;
                    }
                }
            } elseif ($survey_info['land_ceiling_status'] == 'Y') {
                if ($area_unit && $purchased_area) {
                    $applied_purchased_area_unit_value = \App\Models\Common\ConversionModel::select('convers_value_acer', 'convers_value_heactor')->where(['code_id' => "$area_unit", 'fl_archive' => 'N'])->get()->toArray();
                    // t($purchased_area_unit_value);
                    if ($area_unit != 'CD00094') {
                        $applied_total_ceiling_area_acre += $purchased_area * $applied_purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $applied_total_ceiling_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $applied_total_ceiling_area_hectare += $purchased_area * $applied_purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $applied_total_ceiling_area_hectare += $purchased_area;
                    }
                }
            }
        }
        $returnArray['obtaine_grand_total_ceiling_area_acre'] = $obtain_total_ceiling_area_acre;
        $returnArray['obtaine_grand_total_ceiling_area_hectare'] = $obtain_total_ceiling_area_hectare;
        $returnArray['applied_grand_total_ceiling_area_acre'] = $applied_total_ceiling_area_acre;
        $returnArray['applied_grand_total_ceiling_area_hectare'] = $applied_total_ceiling_area_hectare;
        return $returnArray;
    }
