...
class AdminController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $request->session()->put('user_type', $this->data['user_type']);
        ...
        $cond = " fl_archive = 'N' ";
        if ($this->user_type !== 'admin') {
            $sql = 'SELECT STRING_AGG(DISTINCT district_id,\',\') as district FROM "T_ASSGINED_STATE_DISTRICTS" WHERE user_id = \''.$current_user_id.'\' GROUP BY user_id';
            $userDistricts = DB::select($sql);
            // dd($userDistricts);
            if(!empty($userDistricts[0]->district)){
                $districts = $userDistricts[0]->district;
                $implodeText = '\''.implode('\',\'',explode(',',$districts)).'\'';
                $cond .= " AND district_id IN ($implodeText)";

                $sql = $this->getSQL($cond);
        ...
        if ($states) {
            foreach ($states as $key => $val) {
                ...
                //        $grand_total_tot_area = RegistrationModel::getTotalAreaCalculationTwo($finalcond);
                // $grand_total_tot_area = array();

                $grand_total_tot_cost = RegistrationModel::join('T_PAYMENT', 'T_PAYMENT.registration_id', '=', "T_REGISTRATION.id", 'left')->selectRaw('sum(amount) as grand_total_tot_cost')->whereRaw($finalcond)->get()->toArray();
                // $grand_total_tot_cost = array();

                // t($grand_total_tot_area);
                $states[$key]['purchased_area_in_acre'] = $grand_total_tot_area['grand_total_purchase_area_acer'];
                $states[$key]['purchased_area_in_hector'] = $grand_total_tot_area['grand_total_purchase_area_hector'];
                $states[$key]['total_cost'] = $grand_total_tot_cost[0]['grand_total_tot_cost'];
                $states[$key]['leased_area_in_acre'] = $grand_total_tot_area['grand_total_lease_area_acer'];
                $states[$key]['leased_area_in_hector'] = $grand_total_tot_area['grand_total_lease_area_hector'];
    ...
    //             }
    //             //print_r($states);
    //             //echo "finalcond__________________________________________________________________";
    //             //print_r($finalcond);
    //             // $grand_total_tot_area = $this->getTotalAreaCalculation($finalcond);
    //             //print_r($finalcond);
    //             /*echo "states__________________________________________________________________";
    // 			print_r($states);
    // 			echo '<br>';
    // 			echo "districts-------------------------------------------------------------------";
    // 			print_r($assignStateDistrictLists);
    // 			die;*/
    //             $grand_total_tot_area = RegistrationModel::getTotalAreaCalculation($finalcond);
    ...
    // 			print_r($assignStateDistrictLists);
    // 			echo "<br/>-------";
    // 			print_r($grand_total_tot_cost[0]['grand_total_tot_cost']);
    // 			echo "<br/>";*/
    //             $grand_total_cost_all += $grand_total_tot_cost[0]['grand_total_tot_cost'];
    //             //print_r($grand_total_cost_all);
    //             //die; 
    //         }
    //     }
    //     //print_r($states);
    ...
    //     //die;
    //     //print_r($this->data);
    //     //print_r($this->data['grand_total_cost_all']);
    //     //die;
    //     return view('admin.dashboard.index', $this->data);
    // }

    public function getSQL($cond)
    {
        return "SELECT 
                    MAX(state_id) as state_id,
                    MAX(state_name) as state_name,
                    SUM(purchased_area_in_acre) as purchased_area_in_acre,
                    SUM(leased_area_in_acre) as leased_area_in_acre,
                    SUM(purchased_area_in_hector) as purchased_area_in_hector,
                    SUM(leased_area_in_hector) as leased_area_in_hector,
                    SUM(payment_sum) as total_cost
                FROM public.reg_cons_view
                WHERE {$cond}
                GROUP BY state_id
                ORDER BY MAX(state_name)";
    }
        ...
        foreach ($grand_total_tot_area as $key => $value) {
            ...
            if ($tot_area_unit && $total_area) {
                ...
                if ($tot_area_unit != 'CD00094') {
                    $convert_val_acre = isset($tot_area_unit_value[0]['convers_value_acer']) ? $tot_area_unit_value[0]['convers_value_acer'] : 0;
                    // $tot_area_unit_value_acer += $total_area * $tot_area_unit_value[0]['convers_value_acer'];
                    $tot_area_unit_value_acer += $total_area * $convert_val_acre;
                } else if ($tot_area_unit == 'CD00094') {
                    $tot_area_unit_value_acer += $total_area;
                }
                if ($tot_area_unit != 'CD00118') {
                    $convert_val_hector = isset($tot_area_unit_value[0]['convers_value_heactor']) ? $tot_area_unit_value[0]['convers_value_heactor'] : 0;
                    $tot_area_unit_value_hector += $total_area * $convert_val_hector;
    ...
    public function classification(Request $request)
    {
        ...
        $this->data['RegDataWithState'] = isset($indivisualTotal) ? $indivisualTotal : [];
                    ...
                    if (!empty($val)) {
                        ...
                        if ($is_archived == 'N') {
                            ...
                            if ($val['tot_area_unit'] != 'CD00094') {
                                $area_unit_value = isset($tot_area_unit_value[0]['convers_value_acer']) ? $tot_area_unit_value[0]['convers_value_acer'] : 0;
                                $tot_area_unit_value_acer += $val['tot_area'] * $area_unit_value;
                            } elseif ($val['tot_area_unit'] == 'CD00094')
                                $tot_area_unit_value_acer += $val['tot_area'];

                            if ($val['tot_area_unit'] != 'CD00118') {
                                $area_unit_value_hector = isset($tot_area_unit_value[0]['convers_value_heactor']) ? $tot_area_unit_value[0]['convers_value_heactor'] : 0;
                                $tot_area_unit_value_hector += $val['tot_area'] * $area_unit_value_hector;
    ...
    public function dashboardTotal(Request $request)
    {
        ...
        if ($filter == 'District') {
            ...
            $RegDataWithState = DistrictModel::where(['fl_archive' => "N"])->orderBy('district_name')->pluck($select, 'id')->toArray();
        }
        if ($filter == 'Legal Entity') {
            $cond .= " AND legal_entity <> '' ";
            $select = 'legal_entity';
            $RegDataWithState = CodeModel::where(['cd_type' => "legal_entity", 'cd_fl_archive' => 'N'])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        }
        if ($filter == 'Plot Type') {
            $cond .= " AND plot_type_id <> '' ";
            $select = 'plot_type_id';
            $RegDataWithState = CodeModel::where(['cd_type' => "plot_type", 'cd_fl_archive' => 'N'])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        }
        if ($filter == 'Block/Taluk') {
            $cond .= " AND block_id <> '' ";
            $select = 'block_name';
            $RegDataWithState = BlockModel::where(['fl_archive' => "N"])->orderBy('block_name')->pluck($select, 'id')->toArray();
        }
        if ($filter == 'Purchase Type') {
            $cond .= " AND purchase_type_id <> '' ";
            $select = 'purchase_type_id';
            $RegDataWithState = CodeModel::where(['cd_type' => "purchase_type", 'cd_fl_archive' => 'N'])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        }
        if ($filter == 'Purchasing Team') {
            $cond .= " AND purchasing_team_id <> '' ";
            $select = 'purchasing_team_id';
            $RegDataWithState = CodeModel::where(['cd_type' => "purchasing_team", 'cd_fl_archive' => 'N'])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        }
        if ($filter == 'Purpose') {
            $cond .= " AND land_usage <> '' ";
            $select = 'land_usage';
            $allPurpose = CodeModel::where(['cd_type' => "land_usage", 'cd_fl_archive' => 'N'])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        }
        if ($filter == 'Classification') {
            $cond .= " AND plot_classification <> '' ";
            $select = 'land_usage';
            $allPurpose = CodeModel::where(['cd_type' => "plot_classification", 'cd_fl_archive' => 'N'])->orderBy('cd_desc')->pluck('cd_desc', 'id')->toArray();
        }
    ...
    public function get_plant_json()
    {
        $sql = "SELECT row_to_json(fc) as geojson
		FROM (SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features
		FROM (SELECT 'Feature' As type
		, ST_AsGeoJSON(lg.wkb_geometry)::json As geometry
		, row_to_json(lp) As properties
		FROM \"T_LOCATION\" As lg 
		INNER JOIN (SELECT ogc_fid as id FROM \"T_LOCATION\"";
        $sql .= " where 1=1 AND (wkb_geometry is not null)";
        $sql = $sql . ") As lp
		ON lg.ogc_fid = lp.id  ) As f )  As fc;";
        $results = DB::select($sql);
