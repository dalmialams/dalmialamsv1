...
//        t($posted_data,1);
        ...
        if ($context == 'exchange') {
            $total_area_cond .= " and purchase_type_id = 'CD00295'";
            if ($this->user_type == 'admin') {
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
                $allCeilingData = \App\Models\Transaction\LandExchangeModel::selectRaw("array_to_string(array_agg(QUOTE_LITERAL(id)), ',') as id_str")->where($finalcond)->get()->toArray();
            } else {
                if (!empty($state_id) && empty($district_id) && empty($block_id) && empty($village_id)) {
                    $total_area_cond .= " and  district_id in($assignStateDistrictLists)";
                    $allCeilingData = \App\Models\Transaction\LandExchangeModel::selectRaw("array_to_string(array_agg(QUOTE_LITERAL(id)), ',') as id_str")->whereRaw("district_id in ($assignStateDistrictLists)")->get()->toArray();
                } else {
                    $allCeilingData = \App\Models\Transaction\LandExchangeModel::selectRaw("array_to_string(array_agg(QUOTE_LITERAL(id)), ',') as id_str")->where($finalcond)->get()->toArray();
                }
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
            ...
            if ($allCeilingIds) {
                $ceilingSurveyData = \App\Models\Transaction\LandExchangeSurveyModel:: whereRaw("land_exchange_id in ($allCeilingIds)")->get()->toArray();
        ...
        return view('admin.dashboard.context.exchange', $this->data);
    }
   ...
   public function assignStateDistrictLists($state_id = null) {
    ...
    public function surveyTotalAreaDetails($survey_id_str) {
        ...
        $total_exchange_area_acre = 0;
        $total_exchange_area_hectare = 0;
            ...
            $purchased_area = isset($survey_info['purchased_area']) ? $survey_info['purchased_area'] : '';
                ...
                if ($area_unit && $purchased_area) {
                    ...
                    if ($area_unit != 'CD00094') {
                        $total_exchange_area_acre += $purchased_area * $purchased_area_unit_value[0]['convers_value_acer'];
                    } else if ($area_unit == 'CD00094') {
                        $total_exchange_area_acre += $purchased_area;
                    }
                    if ($area_unit != 'CD00118') {
                        $total_exchange_area_hectare += $purchased_area * $purchased_area_unit_value[0]['convers_value_heactor'];
                    } else if ($area_unit == 'CD00118') {
                        $total_exchange_area_hectare += $purchased_area;
                    }
                }
           
        }
        $returnArray['grand_total_exchange_area_acre'] = $total_exchange_area_acre;
        $returnArray['grand_total_exchange_area_hectare'] = $total_exchange_area_hectare;
        return $returnArray;
    }
