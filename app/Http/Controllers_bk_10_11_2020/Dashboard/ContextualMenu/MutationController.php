...
class MutationController extends Controller {
    ...
    public function mutationDetails(Request $request) {
        ...
        if ($context == 'mutation') {
            if ($this->user_type == 'admin') {
                //  echo $total_area_cond;exit;
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
                $mutation_details = \App\Models\Transaction\MutationModel::where($finalcond)->get()->toArray();
            } else {
                if (!empty($state_id) && empty($district_id) && empty($block_id) && empty($village_id)) {
                    $total_area_cond .= " and  district_id in($assignStateDistrictLists)";
                    $mutation_details = \App\Models\Transaction\MutationModel::whereRaw("district_id in ($assignStateDistrictLists)")->get()->toArray();
                } else {
                    $mutation_details = \App\Models\Transaction\MutationModel::where($finalcond)->get()->toArray();
                }
                $total_area_details = RegistrationModel::getTotalAreaCalculation($total_area_cond);
            ...
            if ($mutation_details) {
                ...
                //   $survey_total_area_details = SurveyModel::surveyTotalAreaDetails($survey_id_str, 'mutation');
        ...
        return view('admin.dashboard.context.mutation', $this->data);
    }
