    ...
    public function makeMenu($current_user) {
        ...
        Menu::make('MyNavBar', function($menu) {
            ...
            $menu->group(array('prefix' => 'dashboard'), function($search) {
                $search->add('<span class="txt">Dashboard</span>', array('url' => '', 'secure' => true))->data('permission', array('dashboard_access'));
            });
			$menu->group(array('prefix' => 'map'), function($search) {
                $search->add('<span class="txt">Map</span>', array('url' => 'view', 'secure' => true))->data('permission', array('map_access'));
            });
            //  ->data('permission', array('lis','view'))
            //search
            $menu->group(array(), function($search_menu) {
                $search = $search_menu->add('Search', array('url' => '#', 'secure' => true))->nickname('search')->data('permission', array());
                ...
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("registration_access", $this->current_user_id)) {
                    $search_menu->group(array('prefix' => 'land-details-entry'), function($master) {
                        $master->search->add('Land Record', array('url' => 'registration/list', 'secure' => true))->data('permission', array('registration_search'));
                    });
                }
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("patta_access", $this->current_user_id)) {
                    $search_menu->group(array('prefix' => 'land-details-entry'), function($master) {
                        $master->search->add('Patta Record', array('url' => 'patta/list', 'secure' => true))->data('permission', array('patta_search'));
                    });
                }
            ...
            $menu->group(array(), function($entry_menu) {
                $entry = $entry_menu->add('Entry', array('url' => '#', 'secure' => true))->nickname('entry')->data('permission', array());
                ...
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("registration_access", $this->current_user_id)) {
                    $entry_menu->group(array('prefix' => 'land-details-entry'), function($master) {
                        $master->entry->add('Land Record', array('url' => 'registration/add', 'secure' => true))->data('permission', array('registration_add'));
                    });
                }
                if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("patta_access", $this->current_user_id)) {
                    $entry_menu->group(array('prefix' => 'land-details-entry'), function($master) {
                        $master->entry->add('Patta Record', array('url' => 'patta/add', 'secure' => true))->data('permission', array('patta_add'));
                    });
                }
            ...
            $menu->group(array(), function($trnxn_menu) { 
                $trnxn = $trnxn_menu->add('Transaction', array('url' => '#', 'secure' => true))->nickname('trnxn')->data('permission', array());
                ...
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Audit Status', array('url' => 'transaction/audit/add', 'secure' => true))->data('permission', array('audit_status_add'));
                });
				 }
				 
				 if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("conversion_to_parent_company_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Conversion to Parent Company', array('url' => 'transaction/conversion-parent-company/add', 'secure' => true))->data('permission', array('conversion_to_parent_company_add'));
                });
				 }
				 
				if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("disputes_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Disputes', array('url' => 'transaction/disputes/add', 'secure' => true))->data('permission', array('disputes_add'));
                });
				}
				
				 if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("hypothecation_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Hypothecation', array('url' => 'transaction/hypothecation/add', 'secure' => true))->data('permission', array('hypothecation_add'));
                });
				 }
//                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
...
//                });
               if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_ceiling_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Land Ceiling', array('url' => 'transaction/land-ceiling/add', 'secure' => true))->data('permission', array('land_ceiling_add'));
                });
			   }
			   
			    if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_conversion_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Land Conversion', array('url' => 'transaction/land-conversion/add', 'secure' => true))->data('permission', array('land_conversion_add'));
                });
				}
				
				 if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_exchange_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Land Exchange', array('url' => 'transaction/land-exchange/add', 'secure' => true))->data('permission', array('land_exchange_add'));
                });
				 }
				 
				  if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_inspection_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Land Inspection', array('url' => 'transaction/inspection/add', 'secure' => true))->data('permission', array('land_inspection_add'));
                });
				  }
				  
				   if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("land_reservation_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Land Reservation', array('url' => 'transaction/land-reservation/add', 'secure' => true))->data('permission', array('land_reservation_add'));
                });
				   }
                ...
                $trnxn_menu->group(array('prefix' => 'lease-management'), function($master) {
                    $master->trnxn->add('Lease', array('url' => 'add', 'secure' => true))->data('permission', array('lease_add'));
                });
					}
					
					 if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("mining_lease_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Mining Lease', array('url' => 'transaction/mining-lease/add', 'secure' => true))->data('permission', array('mining_lease_add'));
                });
					 }
					 
					  if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("mutation_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Mutation', array('url' => 'transaction/mutation/add', 'secure' => true))->data('permission', array('mutation_add'));
                });
					  }
                ...
                $trnxn_menu->group(array('prefix' => 'payment'), function($master) {
                    $master->trnxn->add('Payment', array('url' => 'add', 'secure' => true))->data('permission', array('payment_add'));
                });
					   }
//                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
...
//                });
 ...
 if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("under_operation_access", $this->current_user_id)) {
                $trnxn_menu->group(array('url' => '#', 'secure' => true), function($master) {
                    $master->trnxn->add('Under Operation', array('url' => 'transaction/under-operation/add', 'secure' => true))->data('permission', array('under_operation_add'));
                });
 }
            ...
            //MIS
//            $menu->group(array('url' => '#', 'secure' => true), function($search) {
//                $search->add('MIS', array('url' => 'mis/search', 'secure' => true))->data('permission', array());
//            });

            $menu->group(array(), function($mis_menu) {
                $mis = $mis_menu->add('MIS', array('url' => '#', 'secure' => true))->nickname('mis')->data('permission', array());
               ...
               if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("registration_mis_access", $this->current_user_id)) {
                    $mis_menu->group(array('prefix' => 'mis'), function($master) {
                        //  $master->mis->add('Registration Details', array('url' => 'registration', 'secure' => true))->data('permission', array(''));
                        $master->mis->add('Registration Details', array('url' => 'registration', 'secure' => true));
                    });
                }
               if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("transaction_mis", $this->current_user_id)) {
                    $mis_menu->group(array('prefix' => 'mis'), function($master) {
                        // $master->mis->add('Transaction Details', array('url' => '', 'secure' => true))->data('permission', array(''));
                        $master->mis->add('Transaction Details', array('url' => 'transaction', 'secure' => true));
                    });
                }
            ...
            $menu->group(array('prefix' => ''), function($search) {
                $search->add('Contacts', array('url' => 'contact', 'secure' => true))->data('permission', array());
            });
			}
            //admin
            $menu->group(array(), function($admin_menu) {
                $admin = $admin_menu->add('Admin', array('url' => '#', 'secure' => true))->nickname('admin')->data('permission', array());
                ...
                //if ($this->current_user_type == 'admin') {
					if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("activity_log_access", $this->current_user_id)) {
					 ...
					 $admin_menu->group(array(''), function($master) {
                        $master->admin->add('Activity Log', array('url' => 'user-management/user/log', 'secure' => true))->data('permission', array());
                    });
					}
					
					if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("audit_log_access", $this->current_user_id)) {
					 $admin_menu->group(array(''), function($master) {
                        $master->admin->add('Audit Log', array('url' => 'master/audit/audit-log', 'secure' => true))->data('permission', array());
                    });
					}
                                        	if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("search_log_access", $this->current_user_id)) {
					 $admin_menu->group(array(''), function($master) {
                        $master->admin->add('Search Log', array('url' => 'master/search/search-log', 'secure' => true))->data('permission', array());
                    });
					}
					
                    if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("audit_master_management_access", $this->current_user_id)) {					
					$admin_menu->group(array(''), function($master) {
                        $master->admin->add('Audit Master Management', array('url' => 'user-management/audit/add', 'secure' => true))->data('permission', array());
                    });
					}
					
					if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("form_management_access", $this->current_user_id)) {
					 $admin_menu->group(array(''), function($master) {
                        $master->admin->add('Form Management', array('url' => 'user-management/form/add', 'secure' => true))->data('permission', array());
                    });
					}
					...
					$admin_menu->group(array('prefix' => 'master'), function($master) {
						$master->admin->add('Master Data Management', array('url' => 'data/management', 'secure' => true))->data('permission', array());
					});
					}
				
				if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("master_user_access", $this->current_user_id)) {
                    $admin_menu->group(array(''), function($master) {
                        $master->admin->add('Master User', array('url' => 'master-user/add', 'secure' => true))->data('permission', array());
                    });
				}
				
				if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("role_management_access", $this->current_user_id)) {
                    $admin_menu->group(array(''), function($master) {
                        $master->admin->add('Role Management', array('url' => 'user-management/role/add', 'secure' => true))->data('permission', array());
                    });
				}                  
					/*$admin_menu->group(array(''), function($master) {
						$master->admin->add('Shape File Import', array('url' => 'document-management/document/shape-file-list', 'secure' => true))->data('permission', array('shape_file_access'));
					});*/
					
				if ($this->current_user_type == 'admin' || UtilityModel::ifHasPermission("shape_file_access", $this->current_user_id)) {
				$admin_menu->group(array(''), function($master) {
						$master->admin->add('Shape File Import', array('url' => 'document-management/document/shape-file-list', 'secure' => true))->data('permission', array());
					});
				}
				 ...
				 //if ($this->current_user_type == 'admin') {
                    ...
                    $admin_menu->group(array(''), function($master) {
                        $master->admin->add('User Log', array('url' => 'user-management/user/loginlog', 'secure' => true))->data('permission', array());
                    });
					 }
					if ($this->current_user_type == 'admin' || $this->current_user_role_name == 'manager' || $this->current_user_role_name == 'auditor' || $this->current_user_role_name == 'superviser') {
						...
						$master->admin->add('User Management', array('url' => 'user-management/user/add', 'secure' => true))->data('permission', array());
						});
					}
					
					/*$admin_menu->group(array(''), function($master) {
                        $master->admin->add('Approval Management', array('url' => 'approval-management/approval/add', 'secure' => true))->data('permission', array());
                    });
					
					if ($this->current_user_role_id == 4 || $this->current_user_role_id == 8 || $this->current_user_role_id == 9) {
						$admin_menu->group(array('class' => ''), function($master) {
							$master->admin->add('Approval', array('url' => 'approval-management/approval/track', 'secure' => true))->data('permission', array());
						});
					}
					
					$admin_menu->group(array(''), function($master) {
                        $master->admin->add('Document Management', array('url' => 'document-management/document/list', 'secure' => true))->data('permission', array());
                    });*/
            ...
            $menu->group(array('prefix' => ''), function($search) {
                $search->add(' <span class="tooltip" style="font-size: 15px !important;opacity: 1 !important;" data-toggle="tooltip" data-original-title="Logout"><span class="fa fa-power-off"></span>
                        <span class="sr-only">Logout</span></span>', array('url' => 'logout', 'secure' => true))->data('permission', array());
            });
            $menu->group(array('prefix' => ''), function($search) {
                $search->add(' <span class="tooltip" style="margin-left: 10px;font-size: 15px !important;opacity: 1 !important;" data-toggle="tooltip" data-original-title="Change Password"><span class="fa fa-key"></span>
                        <span class="sr-only">Change Password</span>', array('url' => 'change-password', 'secure' => true))->data('permission', array());
            });
